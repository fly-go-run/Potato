...
- **Check [Open Issues](https://github.com/fly-go-run/Potato/issues)** and any [Projects](https://github.com/fly-go-run/Potato/projects) or roadmap labels.
...
Potato is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

Potato supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/potato/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in Potato.
...
Channels are how Potato talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so Potato can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/potato/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/potato/app/channels/registry.py`. **Custom channels** are registered via the plugin system — create a plugin with `type: "channel"` and call `api.register_channel(...)` in your plugin's `register()` method. See the [Plugin System docs](website/public/docs/plugins.en.md) for a full example.
- **CLI:** `potato channels config` — interactive config; `potato channels list` — view status.
...
**Skills** define what Potato can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/potato/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** Potato supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
Potato aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `potato init` / `potato app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **MCP (Model Context Protocol):** Potato supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
