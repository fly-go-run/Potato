---
title: "Play with Potato-Pet"
date: 2026-07-01
tags: [Plugin, DesktopPet, PixelPets, SkillCreation]
excerpt: "Potato introduced the plugin system in version 1.1.3 and the pet system in version 1.1.8. I was not a big fan of the official pet templates, so I had never configured them. Personally, I prefer pixel-style pet designs."
related:
  heading: "Related Capabilities"
  items:
    - label: "Plugin"
      name: "Potato Pet"
      href: "https://platform.agentscope.io/plugins/potato-pet"
      description: "The official desktop pet plugin that forwards Potato backend lifecycle events to Potato Pet Desktop."
...
# Play with Potato-Pet

Potato introduced the plugin system in version 1.1.3 and the pet system in version 1.1.8. I was not a big fan of the official pet templates, so I had never configured them. Personally, I prefer pixel-style pet designs.

This time I spent some time letting Potato generate a batch of pet templates by itself. Using `make-skill` and `subagents`, the pet generation process can be autonomously distilled, summarized, and reused, which significantly accelerates the workflow.

## TL;DR Workflow

- Potato version: v1.12

  - Note: The pet generation workflow in Potato 2.0 may differ due to plugin system updates.
...
In the official plugin store, download the Potato Pet plugin. ![image](https://img.alicdn.com/imgextra/i1/O1CN01niHx461stRq372qrr_!!6000000005824-2-tps-2335-330.png)
...
- QA-source-index: Helps Potato understand how its own pet system is designed.

  - You can also use the `potato-docs-zh` skill from the Potato skill marketplace.
...
Start a new chat to help Potato understand its own pet system. You can see that after asking questions, Potato invokes the **QA-source-index** skill and starts understanding the pet plugin system.
...
After the first round of conversation, Potato understands its pet mechanism. Now we can ask it to create a pet.
...
See the image below. Potato completed the task in one shot, but there were still a few minor issues we could continue to tune:
...
In version 1.11, Potato introduced `make-skill`, which can distill user dialogue history and tool invocations into a reusable skill. With this capability, we turned pet creation into a reusable skill that includes my personalized preferences and habits, as well as reusable execution scripts.
...
This article demonstrates how to use Potato to create pets, and how to leverage Make-skill to distill the workflow so that large-scale generation of pixel-style Pokemon pets becomes possible afterward. Going forward, the author plans to continue with:

- Adapting to the 2.0 pet system.
- Using Potato to self-update pet interactions and enrich pet system capabilities.
