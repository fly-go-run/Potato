...
Before using Potato, you need to configure at least one available model. Potato supports multiple model providers, which you can configure and manage on the **Settings -> Models** page in the left sidebar.

![Settings Models](https://img.alicdn.com/imgextra/i2/O1CN01cmjcv31YO2MD9GzGn_!!6000000003048-2-tps-3810-2064.png)

Potato supports various LLM providers:
...
- [Potato Local (llama.cpp)](https://github.com/ggml-org/llama.cpp)
- [Ollama](https://ollama.com/)
- [LM Studio](https://lmstudio.ai/)

Potato Local (llama.cpp) is built into Potato and does not require additional software installation. Ollama and LM Studio require you to install the corresponding software in advance.

Potato also provides the QwenPaw-Flash series for local deployment. It includes 2B, 4B, and 9B variants, with original, 4-bit, and 8-bit versions for different VRAM and performance requirements. These models are open-sourced on [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) and [Hugging Face](https://huggingface.co/agentscope-ai/models). The following sections explain how to use QwenPaw-Flash with each local provider.

## Potato Local (llama.cpp) Configuration

> Potato Local is currently still in the testing phase, and there may be issues with stability and GPU compatibility. If you are looking for a more stable local model experience or need GPU acceleration, it is recommended to use Ollama or LM Studio as your local model provider in the short term.

Potato Local is a local model provider based on llama.cpp. You can configure and manage it on the **Models** page.

![Potato Local Provider](https://img.alicdn.com/imgextra/i4/O1CN01BBeBIR1zXeIQfAK8b_!!6000000006724-2-tps-3810-2064.png)

When configuring Potato Local for the first time, you need to download the llama.cpp runtime. Click the **Download llama.cpp** button, and Potato will automatically download and configure the runtime. Once the download is complete, you can use the Potato Local provider.

![Download llama.cpp](https://img.alicdn.com/imgextra/i1/O1CN01VhSJKd1yhtWZ0SgYc_!!6000000006611-2-tps-1530-1290.png)

Potato team has trained a series of small models (the QwenPaw-Flash series) suitable for local deployment. Based on your current device (CPU / NVIDIA GPU / Apple M series chip), Potato will automatically recommend suitable model versions for you. If you want to use QwenPaw-Flash, simply choose an appropriate version here, download it, and start it. If you want to use other models, you can add them by entering the _Model Repository ID_ and _Download Source_. The Model Repository ID refers to the identifier of the model in ModelScope / Hugging Face, such as `Qwen/Qwen3-0.6B-GGUF`. The Download Source refers to where the model is downloaded from. Currently, ModelScope and Hugging Face are supported.

![Download Model](https://img.alicdn.com/imgextra/i4/O1CN01ualQNr1KMkbpBiuTa_!!6000000001150-2-tps-1342-1680.png)

After the model is downloaded, you can click the **Start** button to launch the model. The startup time may vary depending on the model size. Once started, Potato will automatically set this model as the global default. Only one model can be running at a time; starting another model will automatically stop the currently running one.
...
Potato Local will automatically record the model's running state. If you close the Potato process while a Potato Local model is running, it will attempt to restart the last used model the next time you open Potato, so you do not need to start the model manually each time.
...
To verify that Ollama is working properly, go to the **Settings** page of the Potato Ollama provider and click the **Test Connection** button.

> For users deploying Potato in a Docker container, if Ollama is installed on the host machine, ensure that the Docker network configuration allows the container to access the host's Ollama service (add `--add-host=host.docker.internal:host-gateway` to the `docker run` command), and set the API address to `http://host.docker.internal:11434`.
...
2. Create a text file named `potato-flash.txt` and replace `/path/to/your/potato-xxx.gguf` with the absolute path of the downloaded `.gguf` file:

```text
FROM /path/to/your/potato-xxx.gguf
TEMPLATE {{ .Prompt }}
RENDERER qwen3.5
PARSER qwen3.5
...
```bash
ollama create potato-flash -f potato-flash.txt
```

4. Go back to the Potato Ollama provider page and click **Discover Models** to add the model to Potato.

After installing and configuring Ollama, go to the **Models** page of the Potato Ollama provider and click **Discover Models** to get the list of available Ollama models. After fetching, you can further click **Test Connection** to verify if the models are working properly.
...
To ensure a good experience in Potato, set the **Default Context Length** to at least 32768 in **Settings -> Model Defaults**, and enable "When applicable, separate `reasoning_content` and `content` in API responses" in **Settings -> Developer -> Experimental Settings**.
...
After completing the above LM Studio configuration, go to the **Settings** page of the Potato LM Studio provider and enter the LM Studio API address, which can be found on the **Developer -> Local Server** page. Be sure to add the `/v1` suffix, e.g., `http://localhost:1234/v1`.
...
```bash
lms import /path/to/your/potato-xxx.gguf -c -y --user-repo AgentScope/QwenPaw-Flash
```

3. Go back to the Potato LM Studio provider page and click **Discover Models** to add the model to Potato.
...
> For users deploying Potato in a Docker container, if LM Studio is installed on the host machine, ensure that the Docker network configuration allows the container to access the host's LM Studio service (add `--add-host=host.docker.internal:host-gateway` to the `docker run` command), and set the API address to `http://host.docker.internal:1234/v1`.
...
> For example, if you deploy vLLM at `http://localhost:8000` and have a model at `/path/to/Qwen3.5`, you can add a custom provider, set the API compatibility mode to OpenAI `chat.completions`, set the Base URL to `http://localhost:8000/v1`, then add a model under this provider with Model ID `/path/to/Qwen3.5` and Model Name `Qwen3.5`. After testing the connection, if everything is configured correctly, you can use this vLLM model in Potato.

## Selecting a Model

Configured model providers and models will appear in the **Settings -> Models -> Default LLM** list. You can select a model as the global default and click the **Save** button on the right. The model set on this page will be used as the global default by Potato. If you do not specify a model in certain scenarios (such as chat), Potato will use the default model set here.

![Default Model Settings](https://img.alicdn.com/imgextra/i1/O1CN01bLNFam20DKL1tYThY_!!6000000006815-2-tps-3810-2064.png)

Since different tasks may require different model capabilities, Potato also supports using different models in different chats. You can select the appropriate provider and model from the dropdown menu in the upper right corner of the **Chat** page. This setting only applies to the current agent and chat. If you do not configure a provider or model in the chat page, Potato will use the global default model.
...
All provider configurations in Potato are saved in the `$POTATO_SECRET_DIR/providers` folder (default `~/.potato.secret/providers`). Built-in provider configurations are in the `builtin` directory, and user-added custom provider configurations are in the `custom` directory. Each provider has a corresponding JSON file named after its ID, e.g., the configuration file for a provider with ID `Qwen` is `Qwen.json`. The file contains the provider's API access information and model list. It is not recommended for regular users to modify these files directly to avoid unnecessary errors. Also, changes to the configuration files require restarting Potato to take effect.

### Local Models

If you use the Potato Local (llama.cpp) provider, Potato will save the llama.cpp runtime, downloaded models and logs in the `$POTATO_WORKING_DIR/local_models` folder (default `~/.potato/local_models`).

- **Runtime**: The llama.cpp runtime is saved in the `$POTATO_WORKING_DIR/local_models/bin` directory. It includes the executable files and related dynamic libraries for llama.cpp. These files are automatically downloaded and configured by Potato. If you have special requirements for llama.cpp (such as needing acceleration capabilities for specific hardware), you can compile your own version of llama.cpp and directly replace the files in this directory.
- **Downloaded Models**: Downloaded models are saved in the `$POTATO_WORKING_DIR/local_models/models` directory. Each model corresponds to a folder named after its ID, for example, the model with ID `Qwen/Qwen3-0.6B-GGUF` will have a folder at `$POTATO_WORKING_DIR/local_models/models/Qwen/Qwen3-0.6B-GGUF`. Inside the model folder, you will find the GGUF file for the model and some metadata files. If you need to use GGUF model files from other sources, you can create a subfolder with the structure `organization/model_name` under the `models` directory, then save the GGUF file in that folder. After refreshing the Potato Local model list, you will see the model in the list (for example, save the `Qwen3-0.6B.gguf` model file to `$POTATO_WORKING_DIR/local_models/models/Qwen/Qwen3-0.6B-GGUF/Qwen3-0.6B.gguf`).
- **Log Files**: Log files are saved in the `$POTATO_WORKING_DIR/local_models/logs` directory. After starting llama.cpp, the log file will be automatically saved as `llama-server.log` in that directory.

### Generation Parameters

Since different models and tasks may require different generation parameters (such as `temperature`, `top_p`, `max_tokens`), Potato supports configuring generation parameters in the provider settings. Go to the provider's **Settings** page, expand **Advanced Configuration**, and enter the parameter configuration in JSON format, for example:
...
After configuring, click **Save**. Potato will automatically include these parameters when generating with models from this provider.
