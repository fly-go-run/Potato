...
In Potato, **heartbeat** means: on a fixed interval, ask Potato the
“questions” you wrote in a file, and optionally send the Potato’s reply to
**the channel where you last chatted**. Good for “regular check-ins, daily
digests, scheduled reminders” — Potato runs without you sending a
message.
...
1. In the current agent’s workspace there is a **heartbeat query file** (default
   name **HEARTBEAT.md**; rename with env **`POTATO_HEARTBEAT_FILE`**). Its
   content is **what to ask Potato on each run** (one or more paragraphs; Potato
...
2. When **`enabled` is true** in config, the system runs on your **every**
   ...
   the user message → Potato replies.
3. **Whether the reply goes to a channel** is set by **target**:
   - **main** — Run Potato only; don’t send the reply to any channel (e.g. local
     self-check, logs).
   - **last** — Send the reply to the **channel/session where you last talked
     to Potato** (e.g. if you last used DingTalk, the heartbeat reply goes to
     DingTalk).
...
`<POTATO_WORKING_DIR>/workspaces/<agent_id>/HEARTBEAT.md`.
Default `POTATO_WORKING_DIR` is `~/.potato` (override with **`POTATO_WORKING_DIR`**);
`<agent_id>` is the current agent id (e.g. `default`).

The default filename is `HEARTBEAT.md`; use **`POTATO_HEARTBEAT_FILE`** to change
...
If you ran `potato init` without `--defaults`, you may be prompted to edit
...
Example (heartbeat on, Potato only, no channel, every 30m) — in that agent’s
**`agent.json`**:
...
> [CLI](./cli) `potato cron create`) instead of heartbeat.
