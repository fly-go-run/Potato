...
The **Console** is Potato's built-in web interface. After running `potato app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with Potato in real time
- Enable/disable/configure messaging channels
- View and manage all chat sessions
- Manage scheduled jobs and heartbeat
- Edit Potato's persona and behavior files
- Enable/import skills to extend Potato's capabilities
...
This is where you talk to Potato. It is the default page when the Console opens.
...
button (↑). Potato replies in real time.
...
Create and manage scheduled jobs that Potato runs automatically by time.
...
The **simplest way to create a cron job is to chat directly with Potato** and let it handle the creation for you. For example, if you want to receive a reminder to drink water on DingTalk, simply message Potato on DingTalk: "Help me create a cron job to remind me to drink water every 5 minutes." Once created, you can view the new task on the Cron Jobs page in the console.
...
2. Fill in each section:
   ...
   - **Task type & content** — **Text**: send fixed text from **Message content**.
     **Agent**: fill **Request content**; on each run Potato receives the text
...
tick, send the contents of `HEARTBEAT.md` as a user message to Potato, and
...
Edit files that define Potato's persona and behavior — `SOUL.md`, `AGENTS.md`,
`HEARTBEAT.md`, etc. — directly in the browser.

> **Multi-agent:** Starting from **v0.1.0**, Potato supports **multi-agent** mode.
> You can run multiple independent agents in one Potato instance, each with its own
...
Manage skills that extend Potato (e.g. read PDF, create Word, fetch news). More
...
Manage runtime environment variables needed by Potato tools and skills (e.g.
`TAVILY_API_KEY`).
...
> **Note:** Variable validity is your responsibility. Potato only stores and loads
> values.
>
...
> Data is stored in `~/.potato/token_usage.json`. Override the filename with
> `POTATO_TOKEN_USAGE_FILE`. See [Config — Environment variables](./config#environment-variables).
...
2. Install the optional Python dependency in the environment that runs Potato:
   `pip install "qwenpaw[whisper]"`.
3. Restart Potato, then select **Local Whisper** here.
