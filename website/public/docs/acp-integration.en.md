...
Potato supports **ACP (Agent Client Protocol)** in two complementary ways:

1. **Potato using ACP as a Tool**: Potato connects to external ACP runners and uses them as delegated collaborators
2. **Potato as an ACP Server**: external clients connect to Potato over ACP

This page explains both modes and the scenarios each one fits best.

---

## Potato using ACP as a Tool

In this mode, Potato acts as an **ACP client / orchestrator**, connecting to **configured and enabled external ACP runners** and bringing them into the current session as delegated collaboration capabilities.

The actual entry point for this mode is the built-in tool `delegate_external_agent`. It is intended for scenarios where Potato needs to collaborate with other ACP-capable external agent runtimes, such as the built-in examples `opencode`, `qwen_code`, `claude_code`, and `codex`. For more ACP-compatible agents, see the official ACP agent list and integration guide: <https://agentclientprotocol.com/get-started/agents>. In other words, Potato does not directly talk to arbitrary external agents. It talks to runners that have been registered in ACP configuration, and it starts, continues, responds to, and closes delegated collaboration sessions through them.

### What this mode does

In this mode, Potato uses the built-in `delegate_external_agent` tool to:
...
Conceptually, this lets Potato treat an external agent as a collaborative, tool-like capability while keeping Potato as the primary orchestrator of the main conversation.
...
Once the command-line side is ready, you can configure a custom runner in Potato or collaborate with one of the built-in runners directly.
...
When an external ACP runner asks for permission, Potato does **not** decide on the user's behalf.
...
This keeps delegated ACP execution aligned with the same user-controlled safety model used elsewhere in Potato.

### When to use ACP as a Tool

Use this mode when:

- you want Potato to collaborate with another agent runtime
- you have a specialized ACP-compatible external runner for a certain class of tasks
- you want Potato to remain the primary orchestrator while delegating part of the work outward

### ACP Tool vs MCP

ACP as a Tool and MCP solve different problems:

- **MCP** connects Potato to external services and tool servers
- **ACP as a Tool** connects Potato to an external **agent** runtime
...
## Potato as an ACP Server

In this mode, Potato exposes itself as an [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) compliant agent service over stdio JSON-RPC. External clients, such as [Zed](https://zed.dev), [OpenCode](https://github.com/nicholasgasior/opencode), or any ACP-compatible editor, can connect to Potato via the `potato acp` command and interact with it programmatically.

### Quick Start

```bash
# Start Potato as an ACP agent
potato acp

# Use a specific agent profile
potato acp --agent mybot

# Use a custom workspace directory
potato acp --workspace /path/to/workspace
...
2. **WORKING_DIR config**: read `agents.active_agent` from `config.json` inside `WORKING_DIR` (default `~/.potato`, or `~/.copaw` for legacy installations; overridable via the `POTATO_WORKING_DIR` environment variable)
...
| Aspect               | Potato as an ACP Server                      | Potato using ACP as a Tool                            |
| -------------------- | --------------------------------------------- | ------------------------------------------------------ |
| Potato's role       | Server / target agent                         | Client / orchestrator                                  |
| Connection direction | External client connects to Potato           | Potato connects to an external runner                 |
| Main purpose         | Let editors or external clients drive Potato | Let Potato delegate work to another agent             |
| Typical entry point  | `potato acp`                                 | Delegation tool + ACP runner configuration             |
...
ACP in Potato is not just one feature. It supports both directions:

- **Expose Potato outward** as an ACP server
- **Reach outward from Potato** to external ACP agents as delegated tools

If you are integrating Potato into another client, start with **ACP Server**.
If you want Potato to coordinate with another agent runtime, use **ACP as a Tool**.
