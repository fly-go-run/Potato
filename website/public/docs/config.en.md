...
From **v0.1.0**, Potato supports **multi-agent**. Configuration is split into two layers:
...
The default working directory is `~/.potato`. After running `potato init`, the complete structure looks like:
...
$POTATO_SECRET_DIR/                       # Default ~/.potato.secret
├── providers.json                       # Model provider config and API keys
└── envs.json                            # Environment variables
```

> **Path explanation:** `$POTATO_WORKING_DIR` and `$POTATO_SECRET_DIR` are environment variables, with default values of `~/.potato` and `~/.potato.secret` respectively. They can be customized via environment variables, see "Environment Variables" section below.

### Directory Explanation

**Global Directory (`~/.potato/`)**
...
**Agent Workspace (`~/.potato/workspaces/{agent_id}/`)**
...
> **Persona files:** Agent behavior and personality are defined by persona files. Running `potato init` automatically creates template files based on your chosen language (`zh` / `en` / `ru`). For detailed explanation and management, see [Agent Persona](./persona).
...
| -------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POTATO_WORKING_DIR`      | `~/.potato`        | Working directory root path                                                                                                                                                                                                                                                              |
| `POTATO_SECRET_DIR`       | `~/.potato.secret` | Sensitive data directory (stores `providers.json` and `envs.json`). Docker default is `/app/working.secret`                                                                                                                                                                              |
| `POTATO_KEYRING_ACCOUNT`  | _(auto)_            | OS keychain account name for the master key. Defaults to `master_key`; when `POTATO_WORKING_DIR`/`POTATO_SECRET_DIR` are set (e.g. a dev checkout) it auto-derives a per-install account so a dev install never overwrites the stable install's key. Set explicitly to name a profile. |
| `POTATO_CONFIG_FILE`      | `config.json`       | Config file name (relative to `POTATO_WORKING_DIR`)                                                                                                                                                                                                                                     |
| `POTATO_HEARTBEAT_FILE`   | `HEARTBEAT.md`      | Heartbeat file name (relative to agent workspace)                                                                                                                                                                                                                                        |
| `POTATO_JOBS_FILE`        | `jobs.json`         | Cron jobs file name (relative to agent workspace)                                                                                                                                                                                                                                        |
| `POTATO_CHATS_FILE`       | `chats.json`        | Conversation history file name (relative to agent workspace)                                                                                                                                                                                                                             |
| `POTATO_TOKEN_USAGE_FILE` | `token_usage.json`  | Token usage record file name (relative to agent workspace)                                                                                                                                                                                                                               |

**Other configuration:**

| Variable                             | Default         | Description                                                                  |
| ------------------------------------ | --------------- | ---------------------------------------------------------------------------- |
| `POTATO_LOG_LEVEL`                  | `info`          | Log level (`debug` / `info` / `warning` / `error` / `critical`)              |
| `POTATO_LOG_MAX_SIZE`               | `5MiB`          | Maximum active log size; accepts bytes or suffixes such as `10MB` and `1GiB` |
| `POTATO_LOG_MAX_BACKUPS`            | `3`             | Number of rotated log backups to retain; `0` disables backups                |
| `POTATO_MEMORY_COMPACT_THRESHOLD`   | `100000`        | Character threshold to trigger memory compaction                             |
| `POTATO_MEMORY_COMPACT_KEEP_RECENT` | `3`             | Number of recent messages to keep after compaction                           |
| `POTATO_MEMORY_COMPACT_RATIO`       | `0.7`           | Threshold ratio for triggering compaction (relative to context window size)  |
| `POTATO_WEB_STATIC_DIR`             | _(auto-detect)_ | Default web UI static files path                                             |
| `POTATO_CONSOLE_STATIC_DIR`         | -               | Explicit legacy-console static files override                                |

**Security & Authentication:**

| Variable                     | Default | Description                                        |
| ---------------------------- | ------- | -------------------------------------------------- |
| `POTATO_AUTH_ENABLED`       | `false` | Whether to enable Web console login authentication |
| `POTATO_AUTH_USERNAME`      | -       | Admin username for auto-registration (optional)    |
| `POTATO_AUTH_PASSWORD`      | -       | Admin password for auto-registration (optional)    |
| `POTATO_TOOL_GUARD_ENABLED` | `true`  | Whether to enable tool guard                       |
| `POTATO_SKILL_SCAN_MODE`    | `warn`  | Skill scanning mode (`block` / `warn` / `off`)     |

Example — use a different working dir for this shell:

```bash
export POTATO_WORKING_DIR=/home/me/my_potato
potato app
```
...
1. **Global config** - `~/.potato/config.json` (providers, environment variables, agent list)
2. **Agent config** - `~/.potato/workspaces/{agent_id}/agent.json` (per-agent settings)
...
| `last_api.host`       | string \| null | `null`              | Host address from last `potato app` start                        |
| `last_api.port`       | int \| null    | `null`              | Port from last `potato app` start                                |
...
| `workspace_dir` | string | No       | Workspace path (optional, defaults to `$POTATO_WORKING_DIR/workspaces/{id}`) |
...
> **Model provider configuration** is stored in `$POTATO_SECRET_DIR/providers.json` (default `~/.potato.secret/providers.json`).
> **Environment variables** are stored in `$POTATO_SECRET_DIR/envs.json` (default `~/.potato.secret/envs.json`).

### Agent config (agent.json)

Each agent has an independent `agent.json` in its workspace directory (`~/.potato/workspaces/{agent_id}/`) that stores all of its configuration (channels, tools, heartbeat, MCP, security, etc.). This allows different agents to have completely different configurations without interfering with each other.
...
Potato needs an LLM provider to work. You can set it up in three ways:

- **`potato init`** — interactive wizard, the easiest way
...
| ---------------------------------- | ------------------------ | --------------------------------------------------- | -------------- |
| Potato Local                      | `potato-local`          | _(local)_                                           | _(none)_       |
...
> **Tip:** Run `potato init` and follow the prompts — it will list available
...
> Potato does not verify whether the key is correct or has sufficient quota —
...
- **`potato init`** — prompts "Configure environment variables?" during setup
...
> keys) are valid. Potato only stores and injects them — it does not verify
> correctness.
...
| Directory                                  | Purpose                                           |
| ------------------------------------------ | ------------------------------------------------- |
| `~/.potato/skill_pool/`                   | Local shared pool for built-ins and shared skills |
| `~/.potato/workspaces/{agent_id}/skills/` | Skills present in a specific agent's workspace    |
...
Skill enabled state and configuration are controlled by `~/.potato/workspaces/{agent_id}/skill.json`.

**Manage skills via:**

- Console (Agent → Skills) — Visual management, import, create, enable/disable
- `potato init` (choose all / none / custom during setup)
- `potato skills config` (interactive toggle)
...
Potato has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention.

Memory files are stored in the agent workspace:

| File / Directory                                        | Purpose                                                               |
| ------------------------------------------------------- | --------------------------------------------------------------------- |
| `~/.potato/workspaces/{agent_id}/MEMORY.md`            | Long-lived key information (decisions, preferences, persistent facts) |
| `~/.potato/workspaces/{agent_id}/memory/YYYY-MM-DD.md` | Daily logs (notes, runtime context, auto-generated summaries)         |
...
- Everything lives under **`~/.potato`** by default; override with `POTATO_WORKING_DIR` (and related env vars) if needed.
- From **v0.1.0**, configuration is split into:
  - **Global config** (`~/.potato/config.json`) — providers, environment variables, agent list
  - **Agent config** (`~/.potato/workspaces/{agent_id}/agent.json`) — per-agent settings
...
- LLM providers are globally configured via `potato init` or the Console.
