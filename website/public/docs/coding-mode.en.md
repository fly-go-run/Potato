...
> **Beta Feature**: Coding Mode is Potato's IDE-style workbench for code-centric tasks. It upgrades the Agent from "chat assistant" to "a collaborator that reads and edits your code directly". Still under active iteration — please share feedback on [GitHub](https://github.com/fly-go-run/Potato/issues).
...
- **Import as a copy** — **clone a remote repo**, **copy a local folder**, **upload a zip**, or **create a blank project**. The result lands as a **copy** under `coding_projects/<name>/` inside Potato's workspace. Your original directory is never modified.
...
**Terminal TUI shortcut:** start Potato from a project directory with
`potato .`, or pass an explicit directory with `potato tui /path/to/repo`.
...
In regular mode the Agent only sees a "workspace" with no notion of a specific project. When you turn Coding Mode on, Potato automatically:
...
- **Workspace directory**: Potato's internal folder for configs, session history, memory, etc. — the Agent **should not touch this by default**
