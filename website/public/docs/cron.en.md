...
In Potato, scheduled tasks (cron jobs) let the system run actions
...
2. Fill in fields:
   ...
   - **Task type and content**:
     ...
     - **agent**: send `content.text` to Potato and deliver its response.
...
You can also tell Potato directly in your target channel:
...
See [CLI `potato cron` section](./cli#potato-cron). Common commands:

```bash
potato cron list
potato cron create ...
potato cron state <job_id>
potato cron run <job_id>
potato cron pause <job_id>
potato cron resume <job_id>
potato cron delete <job_id>
```
...
Example (ask Potato every 2 hours and deliver the response):
...
Potato uses five-field cron: **minute hour day month weekday** (no seconds).
...
- [CLI `potato cron` section](./cli#potato-cron) — command reference
