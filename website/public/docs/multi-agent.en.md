...
Potato supports **multi-agent**, allowing you to run multiple independent AI agents in a single Potato instance.
...
Simply put, **multi-agent** lets you run multiple "personas" in one Potato, where each persona:
...
You might use Potato across multiple platforms:
...
After starting Potato, you'll see the **Agent Selector** in the **top-left corner** of the console:
...
**Note**: After deletion, the workspace directory is retained (to prevent accidental data loss). To completely remove it, manually delete the `~/.potato/workspaces/{agent_id}` directory.
...
If you previously used Potato **v0.0.x**, upgrading to **v0.1.0** will **automatically migrate**:
...
2. **Verify Migration**
   ...
   - After starting Potato, check the agent list in console
...
potato skills config

# Enable for specific agent
potato skills config --agent-id abc123
...
```
~/.potato/workspaces/{agent_id}/PROFILE.md
```
...
# Workspace: ~/.potato/workspaces/code_reviewer
...
Workspace: ~/.potato/workspaces/writer_bot
Profile: [Auto-generated detailed capability description]
```
...
potato channels list --agent-id abc123
potato cron list --agent-id abc123
potato skills list --agent-id abc123
...
- `potato channels` - channel management
- `potato cron` - cron jobs
- `potato daemon` - runtime status
- `potato chats` - chat management
- `potato skills` - skill management

**Commands NOT Supporting `--agent-id`** (global operations):

- `potato init` - initialization
- `potato providers` - model providers
- `potato models` - model configuration
- `potato env` - environment variables
...
cp -r ~/.potato/workspaces/abc123 ~/backups/agent-abc123-$(date +%Y%m%d)

# Backup all agents
cp -r ~/.potato/workspaces ~/backups/workspaces-$(date +%Y%m%d)
```
...
Potato also supports spawning ephemeral sub-tasks **within the current project**.
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Coding Mode ON + project_dir is git repo | Creates a **git worktree** under `<project_dir>/.potato/worktrees/`. Subagent works in the isolated worktree.   |
| Coding Mode OFF + workspace is git repo  | Creates a **git worktree** under `<workspace_dir>/.potato/worktrees/`. Subagent works in the isolated worktree. |
...
Potato copies these files into the worktree when it is created, so the
...
```bash
git worktree list
git worktree remove .potato/worktrees/<id>
```
