# Potato Desktop packaging scripts
...
# Output: dist/Potato-Setup-<version>.exe
# Creates two launchers:
#   - Potato Desktop.vbs (silent, no console window)
#   - Potato Desktop (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/Potato.app/Contents/Resources/env"
PYTHONNOUSERSITE=1 PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m potato desktop
```
...
When you **double-click** the .app and nothing appears, the launcher writes stderr/stdout to `~/.potato/desktop.log`. Inspect that file for errors.
...
When users download the Potato macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'Potato' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the Potato app → **Open** → in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings → Privacy & Security**, find the message like *"Potato was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/Potato.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| `build_macos.sh` | One-click: build wheel → build_common → unpack into Potato.app; optional zip. |
