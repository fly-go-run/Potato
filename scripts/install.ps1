# Potato Installer for Windows (self-contained: includes uv download via GitHub)
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$PotatoHome = if ($env:POTATO_HOME) {
    $env:POTATO_HOME
} elseif ($env:QWENPAW_HOME) {
    $env:QWENPAW_HOME
} elseif (Test-Path (Join-Path $HOME ".potato")) {
    Join-Path $HOME ".potato"
} elseif (Test-Path (Join-Path $HOME ".qwenpaw")) {
    Join-Path $HOME ".qwenpaw"
} else {
    Join-Path $HOME ".potato"
}
$PotatoVenv     = Join-Path $PotatoHome "venv"
$PotatoBin      = Join-Path $PotatoHome "bin"
$PythonVersion = "3.12"
$PotatoRepo     = "https://github.com/fly-go-run/Potato.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[potato] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[potato] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[potato] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
Potato Installer for Windows
...
Write-Host "[potato] " -ForegroundColor Green -NoNewline
Write-Host "Installing Potato into " -NoNewline
Write-Host "$PotatoHome" -ForegroundColor White
...
function Prepare-WebApp {
    ...
    $webDest = Join-Path $RepoDir "src\potato\console"
...
$VenvPotato = Join-Path $PotatoVenv "Scripts\potato.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing Potato from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\Potato"
        }
        Write-Info "Installing Potato from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "potato-install-$(Get-Random)"
        try {
            git clone --depth 1 $PotatoRepo $cloneDir
...
if (-not (Test-Path $VenvPotato)) { Stop-WithError "Installation failed: potato CLI not found in venv" }

Write-Info "Potato installed successfully"

# Check web UI availability (for PyPI installs, check the installed package)
if (-not $script:WebUiAvailable) {
    $webUiCheck = & $VenvPython -c "import importlib.resources, potato; p=importlib.resources.files('potato')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($webUiCheck -eq "yes") { $script:WebUiAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $PotatoBin -Force | Out-Null

$wrapperPath = Join-Path $PotatoBin "potato.ps1"
$wrapperContent = @'
...
$PotatoHome = if ($env:POTATO_HOME) {
    ...
    $env:QWENPAW_HOME
} elseif ((Test-Path (Join-Path $HOME ".qwenpaw")) -and
          -not (Test-Path (Join-Path $HOME ".potato"))) {
    Join-Path $HOME ".qwenpaw"
} else {
    Join-Path $HOME ".potato"
}
$RealBin   = Join-Path $PotatoHome "venv\Scripts\potato.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "Potato environment not found at $PotatoHome\venv"
...
$cmdWrapperPath = Join-Path $PotatoBin "potato.cmd"
$cmdWrapperContent = @"
@echo off
REM Potato CLI wrapper — delegates to the uv-managed environment.
set "POTATO_HOME=%POTATO_HOME%"
if "%POTATO_HOME%"=="" if not "%QWENPAW_HOME%"=="" set "POTATO_HOME=%QWENPAW_HOME%"
if "%POTATO_HOME%"=="" if exist "%USERPROFILE%\.qwenpaw" if not exist "%USERPROFILE%\.potato" set "POTATO_HOME=%USERPROFILE%\.qwenpaw"
if "%POTATO_HOME%"=="" set "POTATO_HOME=%USERPROFILE%\.potato"
set "REAL_BIN=%POTATO_HOME%\venv\Scripts\potato.exe"
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use Potato."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "Potato installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$PotatoHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  potato init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  potato app"  -ForegroundColor White -NoNewline; Write-Host "        # start Potato"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "potato uninstall" -ForegroundColor White
