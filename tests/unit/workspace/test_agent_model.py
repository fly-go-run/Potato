...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from potato.config.utils import get_config_path
    from potato.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr("potato.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("potato.config.config.WORKING_DIR", tmp_path)
