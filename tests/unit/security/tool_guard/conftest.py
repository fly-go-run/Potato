...
@pytest.fixture
def mock_config():
    """Patch ``potato.config.load_config`` to return a mock
    ...
    with patch("potato.config.load_config", return_value=app_cfg) as patched:
        yield patched
...
def mock_env_loader():
    ...
    with patch(
        "potato.constant.EnvVarLoader.get_str",
        return_value="",
    ) as patched:
        yield patched
...
def mock_workspace(tmp_path: Path):
    ...
    workspace.mkdir()
    with patch(
        "potato.config.context.get_current_workspace_dir",
