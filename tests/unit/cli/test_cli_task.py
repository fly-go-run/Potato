# -*- coding: utf-8 -*-
"""Tests for the ``potato task`` headless CLI command."""
...
from potato.cli.task_cmd import _read_instruction
...
def test_output_dir_writes_result_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("potato.cli.task_cmd._run_task", _fake_run_task)
...
def test_e2e_cli_no_guard_and_skills_dir(monkeypatch, tmp_path):
    ...
    async def _spy_run_task(**kwargs):
        ...
        captured["env_tool_guard"] = os.environ.get(
            "POTATO_TOOL_GUARD_ENABLED",
        )
        captured["env_skills_dir"] = os.environ.get("POTATO_SKILLS_DIR")
    ...
    monkeypatch.setattr("potato.cli.task_cmd._run_task", _spy_run_task)
...
def test_isolated_workspace_creates_overlay(tmp_path):
    ...
    from potato.cli.task_cmd import _isolated_skills_workspace
    from potato.agents.skill_system import resolve_effective_skills
