...
After running the equivalent of ``potato init --defaults --accept-security``
...
def _isolate_app_module_state() -> object:
    """Prevent this module's ``from potato.app._app import app`` calls
    from leaking import state into sibling tests.

    ``test_app_factory_importable`` / ``test_critical_routes_return_200``
    import ``potato.app._app``, which leaves it in ``sys.modules`` for
    ...
    ``_ensure_potato_app_not_loaded()``, which asserts the app module
    ...
    sys.modules.pop("potato.app._app", None)
    yield
    # teardown: drop the app factory our tests imported, restore prior
    sys.modules.pop("potato.app._app", None)
    if saved_app is not None:
        sys.modules["potato.app._app"] = saved_app
...
def isolated_working_dir(
    ...
    """Point POTATO at a clean tmp working dir for both constant & config.

    ``potato.constant.WORKING_DIR`` is read at import time, so we patch the
    already-imported module attribute as well as the consumers that cached it.
    """
    work_dir = tmp_path / "potato-home"
    work_dir.mkdir(parents=True, exist_ok=True)

    import potato.constant as constant
    ...
    monkeypatch.setenv("POTATO_WORKING_DIR", str(work_dir))
    return work_dir
...
def test_init_defaults_produces_valid_config(
    ...
    from potato.config import Config, get_config_path, save_config
    from potato.config.config import AgentsDefaultsConfig, HeartbeatConfig
    from potato.constant import HEARTBEAT_DEFAULT_EVERY
...
def test_app_factory_importable() -> None:
    ...
    without raising (the app factory is what ``potato app`` serves)."""
    from potato.app._app import app  # noqa: F401
...
async def test_critical_routes_return_200() -> None:
    ...
    import httpx
    from potato.app._app import app
