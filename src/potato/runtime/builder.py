...
:class:`AgentBuilder` fully constructs a :class:`PotatoAgent` for each
request.  It obtains tools from the per-workspace
:class:`PotatoLocalWorkspace` (via ``list_tools``), the system prompt
...
from ..agents.acp.meta import ACP_CODING_PROJECT_META_KEY, get_coding_project_dir
...
class AgentBuilder:
    ...
    async def build_toolkit(
        ...
        *,
        ...
        :class:`PotatoLocalWorkspace` via ``list_tools()``.
    ...
    async def build(  # pylint: disable=too-many-statements,too-many-branches
        ...
        """Construct a fully-wired :class:`PotatoAgent` for one request.

        Integrates all per-workspace registries: PotatoLocalWorkspace
        ...
        from ..agents.react_agent import PotatoAgent
        ...
        agent = PotatoAgent(
            name=agent_config.name or "Potato",
            model=model,
    ...
    def _apply_request_coding_project(
        ...
        raw_project_dir = get_coding_project_dir(request_context)
    ...
    def _build_context_config(agent_config: Any) -> Any:
        """Map Potato's ``ContextCompactConfig`` to AS ``ContextConfig``."""
        ...
        try:
            ...
            # creates a replacement ToolResultBlock, and drops Potato's
    ...
    def _scroll_recall_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        which requires BOTH the ``POTATO_ALLOW_UNSANDBOXED_RECALL`` env var
    ...
    def _scroll_repl_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        ``POTATO_ALLOW_UNSANDBOXED_RECALL`` env var and
