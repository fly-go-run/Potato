...
class GuardedFunctionTool:
    """Permission-checked tool wrapper using potato's tool-guard engine.

    Routes every tool invocation through
    ``potato.security.tool_guard.engine`` which evaluates guard rules and
...
async def _guarded_tool_check_permissions(
    ...
    **_extra_kwargs: Any,
    ...
    """Drive potato's tool-guard engine + ApprovalService for one tool call.
    ...
    """
    del context  # potato's guard doesn't read PermissionContext yet
