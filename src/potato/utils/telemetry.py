...
TELEMETRY_ENDPOINT = (
    "https://potatoelemetry-sukzkbfzhc.ap-southeast-1.fcapp.run"
)
...
def _detect_install_method() -> str:
    """Detect how Potato was installed based on environment signals."""
    from ..constant import EnvVarLoader

    if EnvVarLoader.get_bool("POTATO_RUNNING_IN_CONTAINER"):
        return "docker"
    if EnvVarLoader.get_bool("POTATO_DESKTOP_APP"):
        return "desktop"
    return "pip"
...
def get_system_info() -> dict[str, Any]:
    ...
    - potato_version: Potato version string
    - install_method: How Potato was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running potato (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as potato_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "potato_version": _safe_get(lambda: potato_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current Potato version string."""
    try:
        from ..__version__ import __version__ as potato_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when Potato is upgraded (or downgraded) to a
    version that hasn't been collected before.

    Args:
        working_dir: Path to Potato working directory
    ...
    try:
        ...
        # v1.1 compat: single potato_version field
        return marker_data.get("potato_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("potato_version", "")
                    if old_ver:
