...
def _normalize_working_dir_bound_paths(data: object) -> object:
    ...
    This keeps POTATO_WORKING_DIR effective even if user config files contain
    older hard-coded paths under ``~/.qwenpaw`` or ``~/.copaw``.
    Only rewrites known working-dir-bound keys.
    """
    legacy_roots = tuple(
        (root, str(Path(root).expanduser().resolve()))
        for root in ("~/.qwenpaw", "~/.copaw")
    )
    ...
    def _rewrite_path_value(v: object) -> object:
        if not isinstance(v, str) or not v:
            return v
        for legacy_root_tilde, legacy_root_abs in legacy_roots:
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    potato.channels), filtered by POTATO_ENABLED_CHANNELS or
    POTATO_DISABLED_CHANNELS when set.

    * POTATO_ENABLED_CHANNELS — whitelist (only these channels are active).
    * POTATO_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, POTATO_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("POTATO_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env POTATO_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_potato_running() -> bool:
    """Check if Potato is currently running by checking API availability.

    Returns:
        True if Potato is running, False otherwise
    """
    try:
