...
class AutoReviewConfig(BaseModel):
    ...
    review_model: Optional[ModelSlotConfig] = Field(
        ...
        description=(
            ...
            "Potato discovers a provider-advertised command-review model "
...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is Potato. How can I help you?"
...
class SIPChannelConfig(BaseChannelConfig):
    ...
    language: str = "zh-CN"
    welcome_greeting: str = "你好，我是Potato"
...
class WeChatConfig(BaseChannelConfig):
    ...
    bot_token_file:         Path to persist/load the bot_token
                            (default ~/.potato/wechat_bot_token).
...
class AgentsConfig(BaseModel):
    ...
    transcription_doubao_stream_resource_id: str = Field(
        default="volc.seedasr.sauc.duration",
        description=(
            "OpenSpeech resource id for Doubao streaming ASR. "
            'Default "volc.seedasr.sauc.duration" '
            "(豆包流式语音识别模型 2.0 · 小时版)."
        ),
    )

    web_search_backend: Literal["auto", "hosted", "tavily", "exa"] = Field(
        default="auto",
        description=(
            ...
            '"auto": Exa when EXA_API_KEY is set, else hosted, else Tavily.'
        ),
    )
    ...
    with _BUILTIN_TOOLS_LOCK:
        if _BUILTIN_TOOLS_CACHE is None:
            ...
            try:
                ...
                importlib.import_module("potato.agents.tools")
...
class ToolGuardConfig(BaseModel):
    ...
    auto_denied_rules: List[str] = Field(
        ...
        description=(
            ...
            "set env POTATO_TOOL_GUARD_AUTO_DENIED_RULES=none."
        ),
    )
...
def build_fallback_agent_profile_config(
    ...
    Used by :func:`load_agent_config` and ``potato doctor fix``
...
def load_agent_config(  # pylint: disable=too-many-branches,too-many-statements
    ...
    with _agent_config_lock:
        ...
        # Normalize legacy ~/.qwenpaw- and ~/.copaw-bound paths.
        # This keeps POTATO_WORKING_DIR effective even if existing agent.json
        # contains older hard-coded paths such as "~/.qwenpaw/media".
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default Potato agent",
    ...
    # Migrate existing workspace files from previous default working dirs.
    # QwenPaw is checked before the older CoPaw location.
    old_workspaces = [
        Path(f"~/{name}").expanduser().resolve()
        for name in (".qwenpaw", ".copaw")
    ]
