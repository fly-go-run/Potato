...
class PotatoProvider:
    ...
    async def search(
        ...
        try:
            ...
            raise RuntimeError(
                f"Potato search returned HTTP {e.response.status_code}",
            ) from e
        if not isinstance(body, dict) or not body.get("success", True):
            ...
            raise RuntimeError(f"Potato search failed: {message}")
...
provider = PotatoProvider()

# Compatibility name used by older market integrations.
QwenPawProvider = PotatoProvider
