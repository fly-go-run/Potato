...
_ENC_PREFIX = "ENC:"
_KEYRING_SERVICE = "potato"
_KEYRING_SERVICES_LEGACY = ("qwenpaw", "copaw")
...
def _keyring_account() -> str:
    ...
    via ``POTATO_WORKING_DIR``) would therefore share — and silently
    overwrite — the stable install's master key, leaving the stable install
    unable to decrypt its own secrets.

    Resolution:

    1. Explicit ``POTATO_KEYRING_ACCOUNT`` override always wins (useful for
    ...
    relocated = bool(
        EnvVarLoader.get_str("POTATO_WORKING_DIR")
        or EnvVarLoader.get_str("POTATO_SECRET_DIR"),
    )
    if not relocated:
...
def _should_skip_keyring() -> bool:
    ...
    if EnvVarLoader.get_bool("POTATO_DISABLE_KEYRING"):
        return True
...
def _try_keyring_get() -> Optional[str]:
    ...
    try:
        ...
        def _get():
            ...
            if value:
                return value
            # Backward compatibility: read both previous service names.
            for legacy_service in _KEYRING_SERVICES_LEGACY:
                value = keyring.get_password(legacy_service, account)
                if value:
                    return value
            return None
