# -*- coding: utf-8 -*-
"""
Skill security scanner for Potato.
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``POTATO_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = EnvVarLoader.get_str("POTATO_SKILL_SCAN_MODE") or None
    if env is not None:
        val = env.lower().strip()
...
_WORKING_DIR_CURRENT_NAME = ".potato"
_WORKING_DIR_LEGACY_NAMES = (".qwenpaw", ".copaw")
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        base_dir = Path.home() / _WORKING_DIR_CURRENT_NAME
        for legacy_name in _WORKING_DIR_LEGACY_NAMES:
