# -*- coding: utf-8 -*-
"""Read-only diagnostics for `potato doctor` (no config or disk mutations)."""
...
# Log file opened on app startup (see ``potato.app._app`` lifespan).
APP_LOG_BASENAME = LOG_FILE_BASENAME

# Built-in local llama.cpp provider id; legacy configs may still use
# copaw-local.
_POTATO_LOCAL_PROVIDER_IDS = frozenset(
    {"potato-local", "qwenpaw-local", "copaw-local"},
)
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `potato app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``potato app``), when ``GET /api/doctor/runtime`` returned
    ...
    lines = [
        ...
        f"potato version: {__version__}",
        f"platform: {platform.system()} {platform.machine()}",
        f"doctor_python_environment: {doctor_env}",
    ]
    if server_python_environment is not None:
        lines.append(
            f"potato_python_environment: {server_python_environment}",
        )
    else:
        lines.append(
            "potato_python_environment: "
            + (server_python_note or "(unknown)"),
        )
    lines.append(f"working_dir: {WORKING_DIR}")
    wd_qp = os.getenv("POTATO_WORKING_DIR")
    wd_qwenpaw = os.getenv("QWENPAW_WORKING_DIR")
    wd_copaw = os.getenv("COPAW_WORKING_DIR")
    if wd_qp:
        lines.append(f"POTATO_WORKING_DIR (env): {wd_qp}")
    elif wd_qwenpaw:
        lines.append(
            f"QWENPAW_WORKING_DIR (env, legacy): {wd_qwenpaw}",
        )
    elif wd_copaw:
        lines.append(f"COPAW_WORKING_DIR (env, legacy): {wd_copaw}")
...
def windows_environment_lines() -> list[str]:
    """Windows-specific read-only diagnostics for ``potato doctor``."""
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``potato app`` may run an automatic migration;
    ...
    return (
        ...
        "`potato app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `potato app` once (or see docs / `potato init`). "
...
def browser_automation_notes(cfg: Config | None) -> list[str]:
    ...
    use_default = (
        EnvVarLoader.get_str("POTATO_BROWSER_USE_DEFAULT", "1")
        .strip()
        .lower()
    )
    if use_default in ("0", "false", "no", "off"):
        notes.append(
            "POTATO_BROWSER_USE_DEFAULT is off — browser_use will not "
...
# --- Potato checks (agent.json, channels, MCP, skills, providers) ---
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~potato.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `potato doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def potato_local_llm_deep_notes() -> list[str]:
