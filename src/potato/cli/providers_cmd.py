...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'potato models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'potato models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'potato models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'potato models add-model' to add models, "
        "then 'potato models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      potato models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
    ...
    click.echo(
        ...
        "  potato models set-llm  (select 'potato-local' provider)",
    )
...
def list_local_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'potato models download <repo_id>' to download one.")
        return
