# -*- coding: utf-8 -*-
"""Extensible hooks for `potato doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``potato.doctor``::

    [project.entry-points."potato.doctor"]
    my_pkg = "my_pkg.doctor:doctor_notes"

The groups ``qwenpaw.doctor`` and ``copaw.doctor`` are still discovered when
present (legacy plugins).
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("potato.doctor", "qwenpaw.doctor", "copaw.doctor"):
