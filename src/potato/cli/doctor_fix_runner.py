# -*- coding: utf-8 -*-
"""Conservative filesystem repairs for ``potato doctor fix``.
...
:func:`~potato.cli.doctor_checks.check_cron_jobs_files` (read-only); exits
...
from ..utils.console_static import find_potato_source_repo_root
    ...
    *,
        ...
        for fid in fix_ids:
            if fid in RISKY_FIX_IDS and not yes and not dry_run:
                raise ValueError(
                    ...
                    "Use `potato doctor fix --dry-run --only ...` to preview "
                    "the plan without -y.",
                )
    ...
    if (
        ...
        raise ValueError(
            ...
            "ensure-working-dir in --only or run `potato doctor fix` without "
    ...
    if "rebuild-web-app-npm" in fix_ids:
        ...
        if repo is None:
            skip_msgs.append(
                "rebuild-web-app-npm: only in a Potato source checkout "
    ...
    if "rebuild-console-npm" in fix_ids:
        ...
        if repo is None:
            skip_msgs.append(
                "rebuild-console-npm: only in a Potato source checkout "
                "with ./console/package.json and ./src/potato/",
            )
        elif not shutil.which("npm"):
            ...
            desc = (
                ...
                "(legacy console; use only with POTATO_CONSOLE_STATIC_DIR)"
            )
