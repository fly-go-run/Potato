...
_CLI_COMMAND_NAMES = ("potato", "qwenpaw", "copaw")
...
def _matches_potato_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a current or legacy CLI."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for command_name in _CLI_COMMAND_NAMES
        for pattern in (
            f" -m {command_name} {subcommand}",
            f" {command_name} {subcommand}",
            f"/{command_name} {subcommand}",
            f'{command_name}.exe" {subcommand}',
            f"{command_name}.exe {subcommand}",
        )
    ) or any(
        f"__main__.py {subcommand}" in lowered for subcommand in subcommands
    )


def _is_potato_service_command(command: str) -> bool:
    """Return whether the command line looks like a local Potato app."""
    return _matches_potato_cli_command(command, "app")


def _is_potato_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a current or legacy wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name in _CLI_COMMAND_NAMES or _matches_potato_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local Potato service."""
    if not host:
        return []
