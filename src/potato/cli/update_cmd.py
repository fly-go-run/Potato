...
from .process_utils import (
    ...
    _is_potato_service_command,
    _process_table,
)

_DISTRIBUTION_NAME = "qwenpaw"
_PYPI_JSON_URL = f"https://pypi.org/pypi/{_DISTRIBUTION_NAME}/json"
...
class InstallInfo:
    """Information about the current Potato installation."""
...
class RunningServiceInfo:
    """Detected Potato service endpoint state."""
...
def _fetch_pypi_release_data() -> dict[str, Any]:
    """Fetch Potato release metadata from its stable PyPI project."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest Potato version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest Potato version: {exc}",
        ) from exc
...
def _select_latest_version(
    ...
    *,
    ...
    if include_prerelease:
        ...
        if not version:
            raise click.ClickException(
                "Unable to determine the latest Potato version.",
            )
        return version
    ...
    if not candidates:
        raise click.ClickException(
            "Unable to determine the latest stable Potato version.",
        )
    return str(max(candidates))


def _fetch_latest_version(*, include_prerelease: bool = False) -> str:
    """Fetch the latest published Potato version from PyPI."""
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running Potato HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local Potato service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_potato_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a Potato HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running Potato service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"Potato service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `potato shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING POTATO SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `potato shutdown` will forcibly terminate the current "
        "Potato backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
...
def _run_shutdown_for_update(
    ...
    """Run `potato shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "potato"]
    ...
    click.echo("")
    click.echo("Running `potato shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `potato shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`potato shutdown` failed. Please stop the running Potato "
            "service manually before running `potato update`.",
        )
...
def _build_upgrade_command(
    ...
    *,
    ...
    package_spec = f"{_DISTRIBUTION_NAME}=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from potato.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[potato] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[potato] Updating Potato "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[potato] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[potato] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[potato] Update completed successfully.")
        click.echo(
            "[potato] Please restart any running Potato service "
            "to use the new version.",
        )
    else:
        click.echo(f"[potato] Update failed with exit code {return_code}.")
        click.echo(
            "[potato] Please fix the error above and run "
            "`potato update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool, prerelease: bool) -> None:
    """Upgrade Potato in the current Python environment."""
    ...
    if version_check is False:
        click.echo("Potato is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `potato update`, or rerun "
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `potato shutdown`. "
                "Please stop it manually before running `potato update`.",
            )

    if not yes and not click.confirm(
        f"Update Potato to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
