...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  potato agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  potato agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      potato agents chat --from-agent bot_a --to-agent bot_b --text "..."
      potato agents create --name "Research Bot" --agent-id research_bot
      potato agents delete research_bot
      potato agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      potato agents list
      potato agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `potato agents list`.

    \b
    Examples:
      potato agents delete research
      potato agents delete research --remove-workspace
      potato agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'potato agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'potato chats list' to find existing sessions (optional)
