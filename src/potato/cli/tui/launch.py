# -*- coding: utf-8 -*-
"""Former Potato TUI launch helpers.

The terminal chat UI is no longer a product surface. ``potato tui`` refuses
to start and points users at the desktop app. Helpers below remain for tests
and leftover call sites.
"""
...
def _build_transport(
    *,
    ...
    ``[sys.executable, "-m", "potato", "acp", "--local-diagnostics"]`` --
    ...
    *not* appended here: ``AcpTransport`` appends ``--agent <id>`` itself when
    ...
    description = (
        f"potato acp ({sys.executable} -m potato acp --local-diagnostics)"
    )
    if project_dir:
...
def tui_cmd(
    ...
    """Former terminal chat UI. Potato is desktop-only."""
    from ..main import DESKTOP_ONLY_HINT
