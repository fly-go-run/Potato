...
* ``AgentMessageChunk`` / ``AgentThoughtChunk`` carry a single content block
  whose ``.text`` is already a *delta* (the Potato ACP server emits deltas).
...
def _is_local_file_uri(uri: str) -> bool:
    ...
    ``resource_link`` could drive a one-click browser open. Potato only emits
...
# ``_meta`` key Potato sets on an ``agent_message_chunk`` to mark it as an
# error; mirrors the ACP server's ``ACP_ERROR_META_KEY``.
_ERROR_META_KEY = "potato.error"
...
def _tool_links(content: Any) -> tuple[FileLink, ...]:
    ...
    that carry a ``uri``), e.g. the link Potato emits for
    ``send_file_to_user``.
    """
    if not content:
        return ()
...
def normalize_update(update: Any) -> list[TuiEvent]:
    ...
    if kind == "agent_message_chunk":
        meta = getattr(update, "field_meta", None)
        # Potato reports per-call token usage as an (otherwise empty)
        ...
        if not text:
            return []
        # Potato tags failed turns via ``_meta`` so we can render them as
