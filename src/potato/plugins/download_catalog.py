...
def _get_version_constraint(
    ...
    """Extract a structured ``potato_version`` from a CDN index entry.

    Reuses the same normalization logic as
    ``scripts/pack/generate_plugin_metadata.get_version``:
      1. If ``potato_version`` dict exists, use directly.
      2. Otherwise synthesize from ``min_version`` / ``max_version``.

    Returns ``None`` when no version constraint is declared.
    """
    potato_version = entry.get("potato_version")
    if not isinstance(potato_version, dict):
        potato_version = entry.get("qwenpaw_version")
    if isinstance(potato_version, dict):
        return {
            k: _normalize_ver(str(v))
            for k, v in potato_version.items()
            if k in ("min", "max")
        }
...
def _is_entry_compatible(entry: dict[str, Any]) -> bool:
    """Return True when a CDN index entry supports the running Potato.

    Checks the structured ``potato_version`` field first, then falls
    ...
    manifest_data: dict[str, Any] = {
        "id": plugin_id,
        "version": version,
        "potato_version": version_constraint,
    }
