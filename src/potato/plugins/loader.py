...
def _desktop_python() -> Optional[str]:
    ...
    path = os.environ.get("POTATO_DESKTOP_PY_RUNTIME", "").strip()
...
def _ensure_plugin_site_on_path() -> None:
    ...
    os.environ["POTATO_PLUGIN_SITE"] = site_dir
...
class PluginLoader:
    ...
    def _check_version_compatibility(
        ...
        """Check plugin compatibility with current Potato version.

        Uses left-closed, right-open semantics: ``>=min, <max``.
        When ``potato_version`` is absent, falls back to legacy
    ...
    async def _load_backend_module(
        ...
        try:
            ...
            if manifest.potato_version is not None:
                qv_dict = manifest.potato_version.model_dump()
            else:
                qv_dict = {
    ...
    def _install_requirements(
        ...
        the case for uv-managed venvs created by the Potato script
    ...
    def _install_requirements_frozen(
        ...
        if python is None:
            raise RuntimeError(
                ...
                "(POTATO_DESKTOP_PY_RUNTIME not set). Reinstall Potato "
    ...
    async def unload_plugin(
        ...
        removes the plugin's tools from ``potato.agents.tools``.
