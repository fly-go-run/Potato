...
class PotatoVersionConstraint(BaseModel):
    """Potato version compatibility range (left-closed, right-open).
...
QwenPawVersionConstraint = PotatoVersionConstraint
...
class PluginManifest(BaseModel):
    ...
    potato_version: Optional[PotatoVersionConstraint] = None
    ...
    def _normalise_input(cls, data: Any) -> Any:
        ...
        # QwenPaw-era manifests remain installable during the rename window.
        if "potato_version" not in data and "qwenpaw_version" in data:
            data["potato_version"] = data["qwenpaw_version"]
