...
from potato.exceptions import ModelNotFoundException
...
PROVIDER_POTATO = OpenAIProvider(
    id="potato-local",
    name="Potato Local",
    is_local=True,
...
class ProviderManager:  # pylint: disable=too-many-public-methods
    ...
    def _init_builtins(self):
        self._add_builtin(PROVIDER_POTATO)
    ...
    def _normalize_provider_id(provider_id: str) -> str:
        ...
        Maps the QwenPaw and CoPaw local provider IDs to ``potato-local``.
        """
        if provider_id in {"qwenpaw-local", "copaw-local"}:
            return "potato-local"
        return provider_id
    ...
    def start_local_model_resume(self, local_manager) -> None:
        ...
        task = asyncio.create_task(
            ...
            name="potato-local-model-resume",
        )
    ...
    def _migrate_copaw_config(self) -> None:
        """Migrate QwenPaw/CoPaw local provider config to potato-local."""
        # 1. Migrate active model configuration (only provider_id)
        if (
            self.active_model
            and self.active_model.provider_id
            in {"qwenpaw-local", "copaw-local"}
        ):
            legacy_provider_id = self.active_model.provider_id
            self.active_model.provider_id = "potato-local"
            self.save_active_model(self.active_model)
            logger.info(
                "Migrated active model provider from '%s' to 'potato-local'",
                legacy_provider_id,
            )

        # 2. Migrate stored provider config file
        legacy_config_path = next(
            (
                self.builtin_path / name
                for name in ("qwenpaw-local.json", "copaw-local.json")
                if (self.builtin_path / name).exists()
            ),
            None,
        )
        ...
        try:
            ...
            provider = self.builtin_providers.get("potato-local")
            if not provider:
                return
            ...
            logger.info(
                "Migrated provider config from '%s' to 'potato-local.json'",
                legacy_config_path.name,
            )
        except Exception as exc:
            logger.warning("Failed to migrate legacy local config: %s", exc)
    ...
    def _migrate_legacy_providers(self):
        ...
        if legacy_path.exists() and legacy_path.is_file():
            ...
            if active_model:
                try:
                    # Convert legacy QwenPaw/CoPaw local provider IDs.
                    if active_model.get("provider_id") in {
                        "qwenpaw-local",
                        "copaw-local",
                    }:
                        active_model["provider_id"] = "potato-local"
    ...
    async def _resume_local_model(self, local_manager) -> None:
        ...
        local_models = self.get_provider("potato-local").extra_models
