...
from potato.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from potato.tauri.sidecar_logging import install_sidecar_logging
...
def _bundled_python() -> str:
    ...
    python = (os.environ.get("POTATO_DESKTOP_PY_RUNTIME") or "").strip()
...
def _child_env_with_plugin_site(env: "dict | None") -> "dict | None":
    ...
    """
    site_dir = (os.environ.get("POTATO_PLUGIN_SITE") or "").strip()
    if not site_dir:
        return env
...
def _reexec_as_bundled_python(args: Sequence[str]) -> None:
    ...
    plugin-installed deps (``POTATO_PLUGIN_SITE``) importable.
    """
    python = _bundled_python()
    if not python:
        print(
            "potato-backend is the desktop backend, not a Python "
...
def _install_subprocess_guard() -> None:
    ...
    * On Windows, suppress console windows for child processes that don't pass
    ...
    if getattr(subprocess.Popen, "_potato_guarded", False):
        return
    ...
    setattr(subprocess.Popen, "_potato_guarded", True)


def _ensure_potato_app_not_loaded() -> None:
    if "potato.app._app" in sys.modules:
        raise RuntimeError(
            "potato app imported before desktop CORS origins were set",
        )


def _sync_loaded_potato_constant_cors_origins() -> None:
    constant_module = sys.modules.get("potato.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from potato.constant.CORS_ORIGINS at import time.
    _ensure_potato_app_not_loaded()
    ensure_desktop_cors_origins()
    _sync_loaded_potato_constant_cors_origins()
...
def _run_backend_server(log_level: str) -> None:
    ...
    from potato.constant import LOG_LEVEL_ENV, WORKING_DIR
    from potato.utils.logging import (
        SuppressPathAccessLogFilter,
        setup_logger,
    )
    from potato.utils.port import get_stable_port, write_port_file
    ...
    os.environ.pop("POTATO_RELOAD_MODE", None)
    ...
    from potato.tauri.backend_guard import reconcile_singleton_backend

    reconcile_singleton_backend(WORKING_DIR)
    if normalized_log_level in ("debug", "trace"):
        from potato.cli.main import log_init_timings
    ...
    from potato.app._app import app as fastapi_app
...
def main() -> None:
    ...
    # the same guard in cli/app_cmd.py for `potato app`).
    from potato.utils.platform import auto_disable_sandbox_on_windows
