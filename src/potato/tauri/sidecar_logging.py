...
def install_sidecar_logging(log_path: Path) -> Path:
    ...
    _LOG_FILE.write("\n")
    _LOG_FILE.write(
        f"== potato tauri sidecar {datetime.now().isoformat()} ==\n",
    )
    ...
    logging.getLogger("potato.tauri").info(
...
def _add_project_file_handler(log_path: Path) -> None:
    # Import lazily so potato.constant is loaded only after desktop env setup.
    from potato.utils.logging import add_project_file_handler
