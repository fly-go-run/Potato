...
@router.get(
    ...
    description=(
        ...
        'Values: "auto", "hosted", "tavily", "exa". Also reports whether '
...
async def get_web_search_backend() -> dict:
    ...
    exa_key = importlib.import_module(
        "potato.agents.tools.web_search",
    )._exa_api_key
...
def _extract_and_merge_zip(data: bytes, workspace_dir: Path) -> None:
    ...
    tmp_dir = None
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix="potato_upload_"))
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
...
async def download_workspace(request: Request):
    ...
    filename = f"potato_workspace_{agent.agent_id}_{timestamp}.zip"
...
@router.post(
    ...
    description=(
        ...
        "the zip are left unchanged (e.g. potato.db, runtime dirs). "
