...
variable ``POTATO_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``POTATO_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = EnvVarLoader.get_str("POTATO_AUTH_ENABLED", "").strip().lower()
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``POTATO_AUTH_ENABLED``
    is truthy and both ``POTATO_AUTH_USERNAME`` and ``POTATO_AUTH_PASSWORD``
    ...
    username = EnvVarLoader.get_str("POTATO_AUTH_USERNAME", "").strip()
    password = EnvVarLoader.get_str("POTATO_AUTH_PASSWORD", "").strip()
