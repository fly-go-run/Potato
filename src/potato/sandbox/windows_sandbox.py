...
def _save_container_metadata(
    ...
    Args:
        state_dir: Potato state directory (``~/.potato``).
...
class WindowsSandbox:
    ...
    async def __aenter__(self):
        ...
        if not _is_admin():
            print(
                "[Potato Sandbox] WARNING: Not running as administrator. "
                "Sandbox ACL setup may fail.",
            )

        fingerprint = _compute_acl_fingerprint(self._config)
        self._container_name = f"potato_{fingerprint[:12]}"
        ...
        if existing_sid:
            ...
            print(
                f"[Potato Sandbox] Reusing existing sandbox "
            ...
            print(
                f"[Potato Sandbox] Sandbox '{self._container_name}' "
...
def shutdown_cleanup() -> None:
    ...
    For each container metadata file in ~/.potato/containers/:
