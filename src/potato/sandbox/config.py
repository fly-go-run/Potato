...
This is the natural entry point into the :mod:`potato.sandbox` package:
...
Backend layout (``create_sandbox`` dispatches to these by ``SandboxMode``):
  - SEATBELT     → mod:`potato.sandbox.macos_sandbox`      (MacOSSandbox)
  - BUBBLEWRAP   → mod:`potato.sandbox.bubblewrap_sandbox` (BubblewrapSandbox)
  - LANDLOCK     → mod:`potato.sandbox.linux_sandbox`      (LinuxSandbox)
  - APPCONTAINER → Windows sandbox, dispatched on ``allow_read_all``:
      True  → windows_restricted_sandbox (WindowsRestrictedSandbox)
      False → mod:`potato.sandbox.windows_sandbox`            (WindowsSandbox)
  - NONE         → mod:`potato.sandbox.local_sandbox`      (NoneSandbox)

Shared base class for all backends:
  - :class:`potato.sandbox.local_sandbox.LocalSandbox`
