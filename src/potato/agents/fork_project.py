...
WORKTREE_REL = Path(".potato") / "worktrees"
REGISTRY_REL = Path(".potato") / "fork_registry.json"
...
INTEGRATION_PROJECT_REL = Path(".potato") / "fork_integration_project"
ACTIVE_SCOPE_REL = Path(".potato") / "fork_active_scope"
_LEGACY_WORKTREE_RELS = (
    Path(".qwenpaw") / "worktrees",
    Path(".copaw") / "worktrees",
)
_LEGACY_REGISTRY_RELS = (
    Path(".qwenpaw") / "fork_registry.json",
    Path(".copaw") / "fork_registry.json",
)
_LEGACY_INTEGRATION_PROJECT_RELS = (
    Path(".qwenpaw") / "fork_integration_project",
    Path(".copaw") / "fork_integration_project",
)
_LEGACY_ACTIVE_SCOPE_RELS = (
    Path(".qwenpaw") / "fork_active_scope",
    Path(".copaw") / "fork_active_scope",
)
    ...
    *,
    ...
    for base in bases:
        if base in seen:
            continue
        seen.add(base)
        for worktree_rel in (WORKTREE_REL, *_LEGACY_WORKTREE_RELS):
...
def _read_path_with_legacy_fallback(
    root: Path,
    current_rel: Path,
    legacy_rels: tuple[Path, ...],
) -> Path:
    """Prefer the Potato path, then an existing legacy brand path."""
...
def project_dir_from_worktree(worktree_path: Path) -> Path:
    """``<project>/.potato/worktrees/<id>`` → ``<project>``."""
...
def _fork_finalize_lock_path(project_dir: Path, branch: str) -> Path:
    ...
    return project_dir / ".potato" / "fork_locks" / f"{digest}.lock"
...
def resolve_integration_project_dir(
    ...
    try:
        ...
        return None
    pointer = _read_path_with_legacy_fallback(
        ws,
...
def _read_active_scope(workspace_dir: Path) -> str:
    path = _read_path_with_legacy_fallback(
        workspace_dir,
        ACTIVE_SCOPE_REL,
        _LEGACY_ACTIVE_SCOPE_RELS,
    )
...
def _ensure_git_identity(wt: Path) -> None:
    ...
    Disabled when ``POTATO_FORK_AUTO_GIT_IDENTITY=0``. Prefers existing
    repo/global git config and does not overwrite a configured identity.
    """
    if os.environ.get("POTATO_FORK_AUTO_GIT_IDENTITY", "1") == "0":
        return
    for key, value in (
        ("user.email", "potato-fork@localhost"),
        ("user.name", "Potato Fork"),
    ):
