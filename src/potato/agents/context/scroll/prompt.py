...
:class:`potato.runtime.prompt_contributors.ScrollContextContributor`). It
...
DISCIPLINE:
  ...
  • Memory files (MEMORY.md / PROFILE.md via read_file; daily and digest
    notes via memory_search) hold the durable preferences, profile facts, and
    decisions you distilled as worth keeping — a quick first reference, a
    curated subset of that same history. For the raw record of what was said,
    asked, done, or decided, recall is the source of truth; memory is not.
"""
...
纪律（DISCIPLINE）：
  ...
  • 记忆文件（MEMORY.md / PROFILE.md 用 read_file 直读；每日与摘要笔记用
    memory_search 检索）保存的是你提炼出来、值得长期保留的偏好、画像事实与
    决策——一个可以先查的快速参考，是同一份历史里精选出的子集。至于“到底说过、
    问过、做过或决定过什么”的原始记录，recall 才是真相来源，memory 不是。
"""
