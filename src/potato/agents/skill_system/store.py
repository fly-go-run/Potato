...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "potato", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.potato.emoji."""
    if not isinstance(metadata, dict):
        return ""
    potato = metadata.get("potato")
    if isinstance(potato, dict):
        return str(potato.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="potato_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"potato_skill_stage_{skill_name}_"),
    )
