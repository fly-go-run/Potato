...
web_search backends, chosen by ``agents.web_search_backend``:
Exa (``EXA_API_KEY``), a host-run Responses search, and keyless Tavily.
``auto`` prefers Exa when a key exists, then hosted, then Tavily.
...
def _exa_api_key() -> str:
    """Exa key from the process env. Never stored in agent config."""
    try:
    ...
    return (
        ...
        or os.environ.get("POTATO_EXA_API_KEY")
        or ""
    ).strip()


def _format_exa_results(results: list) -> str:
    if not results:
        return "No results found."
    lines: list[str] = []
    for index, item in enumerate(results, 1):
        ...
        if snippet and not highlights:
            lines.append(f"    {snippet[:500]}")
        lines.append("")
    return "\n".join(lines).rstrip()


async def _search_exa(query: str) -> ToolChunk:
    """Exa search: titles, URLs, and highlights."""
    key = _exa_api_key()
    if not key:
        text = (
        ...
        return ToolChunk(
            ...
            metadata={
                "qp": build_qp_meta("web_search", False, {"backend": "exa"}),
            },
        )
    try:
        data = await _post(
            ...
            payload={
                "query": query,
                "type": "auto",
                "numResults": _DEFAULT_MAX_RESULTS,
        ...
        if not isinstance(results, list):
            results = []
        text = _format_exa_results(results) or "No content searched."
        return ToolChunk(
            is_last=True,
            state=ToolResultState.SUCCESS,
            content=[TextBlock(type="text", text=_as_untrusted(text))],
        ...
        logger.warning("web_search via Exa failed: %s", exc)
