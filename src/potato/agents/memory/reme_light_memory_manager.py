...
def _to_reme_session_id(session_id: str) -> str:
    ...
    ReMe 0.4 uses ``session_id`` as a filename component. Potato channel
...
class ReMeLightMemoryManager(BaseMemoryManager):
    ...
    ReMe uses the Potato workspace root as its vault.  Daily memory,
    ...
    async def start(self) -> None:
        ...
        await self._update_potato_model()
        try:
    ...
    async def _update_potato_model(self) -> None:
        """Reuse Potato's active model in ReMe's default LLM component."""
        if self._reme is None:
            return
