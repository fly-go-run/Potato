# -*- coding: utf-8 -*-
"""Embedded ReMe application configuration for Potato memory.

ReMe's standalone CLI normally loads YAML such as
``reme/config/default.yaml`` or ``reme/config/potato.yaml``.  Potato embeds
...
from potato.config.config import AgentProfileConfig, EmbeddingModelConfig
...
def build_reme_app_config(
    *,
    ...
    """Build ReMe ``Application`` kwargs for embedded Potato usage."""
...
def _base_config() -> dict[str, Any]:
    """Return the ReMe config shape used by Potato."""
...
def _apply_embedding_config(
    ...
    """Map Potato embedding config into ReMe component config."""
