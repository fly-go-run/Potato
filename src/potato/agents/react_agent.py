# -*- coding: utf-8 -*-
"""Potato Agent - Main agent implementation.

This module provides the main PotatoAgent class built on ReActAgent,
...
class PotatoAgent(CodingModeMixin, Agent):
    """Potato Agent with integrated tools, skills, and memory management.
    ...
    def __init__(
        ...
        *,
        ...
        # Bypass agentscope's built-in permission engine — potato uses
    ...
    def load_state_dict(self, state_dict: dict, strict: bool = True) -> None:
        ...
        if isinstance(memory_raw, dict):
            from potato.app.chats.utils import parse_legacy_memory_state
    ...
    def _set_formatter_media_strip(self, enabled: bool) -> None:
        ...
        if formatter is None:
            return
        setattr(formatter, "_potato_force_strip_media", enabled)
    ...
    def _is_bad_request_or_media_error(exc: Exception) -> bool:
        ...
        if PotatoAgent._is_content_safety_error(exc):
            return False
    ...
    async def _reply(self, **kwargs: Any) -> Any:
        ...
        source = (self._request_context or {}).get("source")
        session_id = (self._request_context or {}).get("session_id", "")
        if source != "job_wakeup" and session_id:
            try:
                from ..runtime.jobs import get_job_registry

                get_job_registry().reset_wake_budget(session_id)
...
# QwenPaw-era plugins imported this class by name and may still patch it.
QwenPawAgent = PotatoAgent
