...
potato envelope schema and the ACP transport stack.  Eagerly importing
them here would slow down every consumer of ``potato.agents.acp`` —
including ``delegate_external_agent`` which only needs
``tool_adapter`` — so we expose ``PotatoACPAgent`` /
``run_potato_agent`` / ``ACPService`` / ``*_acp_service`` via
...
_LAZY_EXPORTS = {
    "PotatoACPAgent": ".server",
    "run_potato_agent": ".server",
    "ACPService": ".service",
...
# fine — verified by ``from potato.agents.acp import ACPService`` working.
__all__ = [
    "ACPErrors",
    "ACPConfigurationError",
    "ACPProtocolError",
