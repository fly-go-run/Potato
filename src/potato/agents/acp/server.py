# -*- coding: utf-8 -*-
"""Potato ACP Agent server.

Exposes Potato as an ACP-compliant agent that external clients
...
ACP_ERROR_META_KEY = "potato.error"
ACP_AGENT_META_KEY = "potato.agent"

_GENERIC_PROMPT_ERROR = (
    "Potato failed to process the request. Check server logs for details."
)
...
class PotatoACPAgent(Agent):
    ...
    def _session_info(
        *,
        ...
        info: dict[str, Any] = {
            ...
            "mode": PotatoACPAgent.MODE_DEFAULT,
        }
        project_dir = get_coding_project_dir(meta)
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        async with self._workspace_lock:
            ...
            logger.info(
                "Potato ACP Agent workspace started:"
    ...
    async def prompt(  # pylint: disable=too-many-locals,unused-argument
        ...
        **kwargs: Any,
        ...
        project_dir = get_coding_project_dir(session_info)
        if isinstance(project_dir, str):
            project_dir = project_dir.strip()
            if project_dir:
    ...
    async def _bridge_approval_requests(
        ...
        *,
        ...
        """Bridge Potato ApprovalService waits to ACP permission prompts."""
    ...
    async def _request_approval_decision(
        ...
        """Ask the ACP client to approve/deny a Potato pending approval."""
        from ...app.approvals import get_approval_service
    ...
    def _approval_options(pending: Any) -> list[PermissionOption]:
        """Build ACP permission options for a pending Potato approval."""
        display = PotatoACPAgent._approval_display(pending)
        if (
    ...
    def _session_meta(self) -> dict[str, Any] | None:
        """Return session ``_meta`` with the resolved Potato agent id."""
        try:
            agent_id = self._resolve_agent_id()
        except Exception:
...
async def run_potato_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
    local_diagnostics: bool = False,
) -> None:
    """Entry point: run Potato as an ACP agent over stdio."""
    agent = PotatoACPAgent(
        agent_id=agent_id,
...
# Compatibility exports for ACP clients integrated before the Potato rename.
QwenPawACPAgent = PotatoACPAgent
run_qwenpaw_agent = run_potato_agent
