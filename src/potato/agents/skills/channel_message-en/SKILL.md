...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with potato chats list, then push with potato channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `potato channels send`**
...
```bash
potato chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
potato chats list --agent-id <your_agent> --user-id <user_id>
```
...
`potato channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`potato channels send` only sends — it does not wait for a reply.
...
```bash
potato chats list --agent-id notify_bot --channel feishu
...
```bash
potato chats list --agent-id task_bot --channel console
...
```bash
potato chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `potato channels send`.
...
```bash
potato chats list --agent-id <your_agent>
```
...
```bash
potato channels list --agent-id <your_agent>
```
...
- **potato agents chat**: sends to another agent, two-way, returns a reply
- **potato channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `potato agents chat`
- Need to proactively push a message to a user → `potato channels send`
...
- `--target-user`: target user ID (obtained from `potato chats list`)
- `--target-session`: target session ID (obtained from `potato chats list`)
