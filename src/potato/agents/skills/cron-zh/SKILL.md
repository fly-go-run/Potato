---
name: cron
description: 仅在需要未来定时执行或周期执行任务时，使用本 skill。使用 potato cron list/create/get/state/update/pause/resume/delete/run 管理任务，并始终显式传入 --agent-id。
metadata:
...
- 缺少参数时，先问用户再创建
- 修改/暂停/删除前，先 `potato cron list --agent-id <agent_id>`
- 排查问题时，用 `potato cron state <job_id> --agent-id <agent_id>`
