---
name: cron
description: Use this skill only for scheduled or recurring tasks. Manage jobs with potato cron list/create/get/state/update/pause/resume/delete/run, and always pass --agent-id explicitly.
metadata:
...
6. **For an agent task that should run without channel output, add `--silent`; execution and session history still continue**
...
All `potato cron` commands **must** include:
...
```bash
# List tasks
potato cron list --agent-id <agent_id>

# View task details
potato cron get <job_id> --agent-id <agent_id>

# View task status
potato cron state <job_id> --agent-id <agent_id>

# Create a task
potato cron create --agent-id <agent_id> ...

# Delete a task
potato cron delete <job_id> --agent-id <agent_id>

# Pause / Resume a task
potato cron pause <job_id> --agent-id <agent_id>
potato cron resume <job_id> --agent-id <agent_id>

# Run an existing task once immediately
potato cron run <job_id> --agent-id <agent_id>

# Update an existing task
potato cron update <job_id> --agent-id <agent_id> ...
```
...
```bash
potato cron create --agent-id <agent_id> -f job_spec.json
```
...
5. Create the task with potato cron create
...
- Before modifying/pausing/deleting, run `potato cron list --agent-id <agent_id>` first
- To troubleshoot issues, use `potato cron state <job_id> --agent-id <agent_id>`
...
- Before creating, you can run `potato chats list --agent-id <agent_id>` to get valid `target-user` and `target-session`
...
potato cron -h
potato cron list -h
potato cron create -h
potato cron get -h
potato cron state -h
potato cron pause -h
potato cron resume -h
potato cron delete -h
potato cron run -h
```
