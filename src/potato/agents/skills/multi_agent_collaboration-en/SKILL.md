...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use potato agents chat for two-way communication with replies.
metadata:
...
```bash
potato agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `potato agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `potato agents chat --background` |
...
2. potato agents list
3. potato agents chat to start a conversation
...
2. potato agents list
3. potato agents chat --background to submit the task
...
`potato agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  potato agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && potato agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
potato agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && potato agents chat --background --task-id <id>
sleep 20 && potato agents chat --background --task-id <id>
sleep 30 && potato agents chat --background --task-id <id>
```
...
```bash
potato agents -h
potato agents list -h
potato agents chat -h
```
