---
name: guidance
description: "Answer user questions about Potato installation and configuration: first locate and read local documentation, then distill the answer; if local information is insufficient, fall back to the official website documentation."
metadata:
...
# Potato Installation and Configuration Q&A Guide

Use this skill when the user asks about **Potato installation, initialization, environment configuration, dependency requirements, or common configuration options**.
...
```bash
DOCS_DIR=$(python3 -c "from potato.constant import DOCS_DIR; print(DOCS_DIR or '')" 2>/dev/null)
```
...
DOCS_DIR=$(find ~/.potato/memory/ -type d -name "docs")
```
...
Run the following script logic to obtain the variable $POTATO_ROOT:

```bash
# Get the absolute path of the binary
POTATO_PATH=$(which potato 2>/dev/null || whereis potato | awk '{print $2}')

# Logical deduction: if the path contains .potato/bin/potato, the root is three levels up
# Example: /path/to/Potato/.potato/bin/potato -> /path/to/Potato
if [[ "$POTATO_PATH" == *".potato/bin/potato" ]]; then
    POTATO_ROOT=$(echo "$POTATO_PATH" | sed 's/\/\.potato\/bin\/potato//')
else
    # Fallback: try to get the parent of the parent directory
    POTATO_ROOT=$(dirname $(dirname "$POTATO_PATH") 2>/dev/null || echo ".")
fi

echo "Detected Potato Root: $POTATO_ROOT"
```

Verify and list the documentation directory:
Use the derived $POTATO_ROOT to locate the documentation:

```bash
# Construct the standard documentation path
DOCS_DIR="$POTATO_ROOT/website/public/docs/"
...
if [ -d "$DOCS_DIR" ]; then
    ...
    find "$POTATO_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```

**If project documentation does not exist, search the working directory**

If documentation is still not found, search for available documentation content under the potato installation path:
