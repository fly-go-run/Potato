...
- This **attaches to an external browser**, not a new process launched by Potato
...
### 1. Managed CDP launched by Potato
...
These browsers are launched and owned by Potato. `stop` will:
...
These browsers are not launched by Potato. `stop` will only:
