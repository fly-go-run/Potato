...
The internal Potato workspace (configs, sessions, memory) is at:
...
class CodingModeMixin:
    ...
    At runtime this class is mixed into ``PotatoAgent`` and combined
    with ``Agent`` via MRO. Coding Mode prompt injection is handled by
    :class:`~potato.runtime.prompt_contributors.CodingModeContributor`.
    """
    ...
    # ------------------------------------------------------------------
    # Tool registration hook (called from PotatoAgent._create_toolkit)
    # ------------------------------------------------------------------
