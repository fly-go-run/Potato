...
class FileLoopGate(LoopGate):
    ...
    def _build_state_dir(
        ...
        """Session-scoped state directory, including legacy run state."""
        current = workspace_dir / ".potato" / "loop_state" / sid
        if current.exists():
            return current
        for legacy_brand in (".qwenpaw", ".copaw"):
            legacy = workspace_dir / legacy_brand / "loop_state" / sid
            if legacy.exists():
                return legacy
        return current
