...
[![GitHub 仓库](https://img.shields.io/badge/GitHub-仓库-black.svg?logo=github)](https://github.com/fly-go-run/Potato)
...
[![最后提交](https://img.shields.io/github/last-commit/fly-go-run/Potato)](https://github.com/fly-go-run/Potato)
...
[![GitHub Star](https://img.shields.io/github/stars/fly-go-run/Potato?style=flat&logo=github&color=yellow&label=Star)](https://github.com/fly-go-run/Potato/stargazers)
[![GitHub Fork](https://img.shields.io/github/forks/fly-go-run/Potato?style=flat&logo=github&color=purple&label=Fork)](https://github.com/fly-go-run/Potato/network)
...
| **本地云端，自由运行**                    | QwenPaw-Flash 模型（2B / 4B / 9B）面向 Agent 场景专项训练。内置 Potato Local 运行时，无需 API Key、不依赖云端。同时支持 Ollama、LM Studio 及 14+ 云端模型供应商。                  |
...
<details>
<summary><b>你可以用 Potato 做什么</b></summary>
...
- [2026-07-10] **v2.0.0 — Potato 2.0 正式发布** 🎉 | 基于 AgentScope 2.0 全面重构，Agent OS 架构、Loop Engineering、Scroll 上下文、ReMe v0.4.0 长期记忆与内置终端界面。
  ...
  基于 Agent OS，我们将陆续推出开箱即用的 Potato 应用小程序——如 **Potato Creator**、**Potato Insight** 等，敬请期待。 [v2.0.0 发布说明 →](https://qwenpaw.agentscope.io/release-notes#v2.0.0)
...
- [2026-06-11] **AgentScope 体验平台上线** — 支持免费部署 Potato、插件分享与 Skill 市场。[立即体验 →](https://platform.agentscope.io/)
...
无需手动配置 Python，一行命令自动完成安装。脚本会自动下载 uv（Python 包管理器）、创建虚拟环境、安装 Potato 及其依赖（含 Node.js 和前端资源）。注意：部分网络环境或企业权限管控下可能无法使用。
...
>    - **找到安装目录**：
>      - 检查 `uv` 是否可用：在 CMD 中输入 `uv --version`，如果显示版本号，则**只需配置 Potato 路径**；如果提示 `'uv' is not recognized as an internal or external command, operable program or batch file,`，则需同时配置两者。
>      - uv 路径（任选其一，取决于安装位置，若 `uv` 不可用则填）：通常在 `%USERPROFILE%\.local\bin`、`%USERPROFILE%\AppData\Local\uv` 或 Python 安装目录下的 `Scripts` 文件夹
>      - Potato 路径：通常在 `%USERPROFILE%\.potato\bin`。
>    - **手动添加到系统的 Path 环境变量**：
...
>   - **配置 `uv` 环境变量**：将 `uv` 所在目录和 `%USERPROFILE%\.potato\bin` 添加到系统的 `Path` 变量中。
>   - **重新运行**：打开新终端，再次执行安装脚本以完成 `Potato` 安装。
>   - **配置 `Potato` 环境变量**：将 `%USERPROFILE%\.potato\bin` 添加到系统的 `Path` 变量中。

安装完成后，请打开新终端并运行：

```bash
potato init --defaults   # 或：potato init（交互式）
potato app
```
...
# 卸载
potato uninstall          # 保留配置和数据
potato uninstall --purge  # 删除所有内容
```
...
>
> Docker 容器内的 `localhost` 指向容器自身，而非宿主机。如果 Ollama（或其他模型服务）运行在宿主机上，可通过以下方式让容器内的 Potato 访问：
>
...
>   agentscope/qwenpaw:latest
> ```
> 然后在 Potato **设置 → 模型** 中，将 Base URL 改为 `http://host.docker.internal:<端口>` — 例如 Ollama 填 `http://host.docker.internal:11434`，LM Studio 填 `http://host.docker.internal:1234/v1`。
>
...
若希望将 Potato 部署在阿里云上，可使用阿里云 ECS 一键部署：打开 [Potato 阿里云 ECS 部署链接](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) 按页面提示操作即可。详细步骤见 [阿里云开发者社区：3 分钟部署你的 AI 助理](https://developer.aliyun.com/article/1713682)。

---

### 方式五：AgentScope Platform 部署

[AgentScope Platform](https://platform.agentscope.io/) 提供一键云端 Potato 部署、插件分享与 Skill 市场。完全免费，7/24 在线。

---

### 方式六：使用魔搭创空间

使用 [魔搭创空间](https://modelscope.cn/studios/fork?target=AgentScope/QwenPaw) 也可进行云端 Potato 部署。注意：请将创空间设为 **非公开**，否则他人可能操纵你的 Potato。
...
如果你不习惯使用命令行，可以下载并使用 Potato 的桌面应用版本，无需手动配置 Python 环境或执行命令。

#### 下载

从 [官方下载页](https://qwenpaw.agentscope.io/downloads) 下载桌面应用（Tauri 版）：
- **Windows**：`Potato-Tauri-<version>-Windows-setup.exe`
- **macOS**：`Potato-Tauri-<version>-macOS.zip`（推荐 Apple Silicon）
...
当你从 Releases 下载 Potato macOS 应用时，macOS 可能显示：*"Apple 无法验证 'Potato' 不包含恶意软件"*。这是因为应用未经过公证。你仍然可以通过以下方式打开：

- **右键打开（推荐）**
  右键点击（或 Control + 点击）Potato 应用 → **「打开」** → 在对话框中再次点击 **「打开」**。这会告诉 Gatekeeper 你信任该应用；之后可以像往常一样双击启动。

- **在系统设置中允许**
  如果仍被阻止，进入 **系统设置 → 隐私与安全性**，向下滚动找到类似 *「已阻止 'Potato'，因为无法验证开发者」* 的提示，点击 **「仍要打开」** 或 **「允许」**。
...
更喜欢待在终端里？运行 `potato` 即可打开一个全屏聊天 **TUI**，它驱动的是与控制台、IM 频道**完全相同**的智能体 — 同一份记忆、技能、MCP 工具与会话 — 无需离开键盘。

```bash
potato                     # 与当前激活的智能体开启对话
potato tui --resume <id>   # 恢复之前的某个会话
potato .                   # 在当前代码仓库中启动（Coding 模式）
```
...
![Potato 终端界面](https://img.alicdn.com/imgextra/i2/O1CN01IULzib1TRAzigIcqG_!!6000000002378-2-tps-2350-1312.png)

---

## API Key

若使用**云端大模型 API**（如 DashScope / 通义千问、OpenAI、Anthropic、Google Gemini、DeepSeek、Kimi、OpenRouter 等），在开始对话前必须配置 API Key。未配置有效 Key 前，Potato 无法正常工作。详情请参考 [官方文档](https://qwenpaw.agentscope.io/docs/models)。

**配置方式：**

1. **控制台（推荐）** — 运行 `potato app` 后，打开 **http://127.0.0.1:8088/** → **设置** → **模型**。选择提供商、填写 **API Key**，并启用该提供商与模型。
2. **`potato init`** — 运行 `potato init` 时，会引导你配置 LLM 提供商与 API Key。按提示选择提供商并填写 Key 即可。
...
> **仅用本地模型？** 若使用 [本地模型](#本地模型)（Potato Local / Ollama / LM Studio），则**无需**任何 API Key。

**接口协议（Chat Completions / Responses）：** 除传统的 `/v1/chat/completions` 外，Potato 也支持 Codex 所用的 Responses 协议（`POST /v1/responses`）。内置 **OpenAI (Response API)** 与 **DeepSeek (Response API)** 两个开箱即用的入口；要接任意 Responses 端点（自建网关、Azure，或 codex 的 `responses-api-proxy`），在 **设置 → 模型 → 添加自定义供应商** 里把 **接口协议** 切到 **Responses**、填上 Base URL 即可，命令行则用 `potato models add-provider <id> -n <名称> -u <base-url> --protocol responses`。

## 本地模型

Potato 可在本机完全本地运行大模型，无需 API Key 或云端服务。详情请见 [官方文档](https://qwenpaw.agentscope.io/docs/models)。

Potato 还提供 **QwenPaw-Flash** 系列 — 面向 Agent 场景专项训练的 2B / 4B / 9B 模型，提供 Q4 与 Q8 量化版本。可在 [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) 与 [Hugging Face](https://huggingface.co/agentscope-ai/models) 获取。

| 后端                              | 适用场景                                 | 安装                                                                 |
| --------------------------------- | ---------------------------------------- | -------------------------------------------------------------------- |
| **Potato Local**（llama.cpp）    | 跨平台（macOS / Linux / Windows）        | 内置；在 Web 界面点击「Download」即可。支持 QwenPaw-Flash，并提供硬件感知推荐。 |
...
- **File Guard** — 独立于 Tool Guard；阻止智能体访问敏感文件与目录（默认保护 `~/.potato.secret/`、`~/.ssh` 等）。
...
| 主题                                                                 | 说明                                             |
| -------------------------------------------------------------------- | ------------------------------------------------ |
| [项目介绍](https://qwenpaw.agentscope.io/docs/intro)                 | Potato 是什么、怎么用                           |
...
Potato 在开放协作中持续演进，欢迎各种形式的参与！请参考上方 [路线图](#路线图)（尤其是标记为 **征集中** 的项）选择你感兴趣的方向，并阅读 [CONTRIBUTING](https://github.com/fly-go-run/Potato/blob/main/CONTRIBUTING_zh.md) 了解如何开始。我们特别欢迎：

- **横向拓展** — 新频道、模型提供商、Skills、MCP。
- **已有功能扩展与完善** — 展示与交互优化、下载提示、Windows 路径兼容等。

欢迎在 [GitHub Discussions](https://github.com/fly-go-run/Potato/discussions) 参与讨论、提出想法或认领任务。
...
- **然后**：运行 `potato init --defaults`，再运行 `potato app`。

> **版本更新提示：** 当执行 `git pull` 更新到大版本后，请重新构建前端、重新安装 Python 包（`pip install -e .`）、重启 `potato app`，并清除浏览器缓存（`Ctrl+Shift+R` 或 macOS 上 `Cmd+Shift+R`）。

---

## 为什么叫 Potato？

Potato 既是 Qwen Personal Agent Workstation（千问个人智能体工作台），也寓意 Qwen 的智识与 Paw 的温度。我们希望它不是冰冷的工具，而是一只随时准备帮忙的智慧、温暖「小爪子」，是你数字生活中最默契的伙伴。
...
在 GitHub 上 Star Potato，第一时间收到新版本发布通知。

---

## 遥测数据

Potato 在执行 `potato init` 时会收集**匿名**使用数据，帮助我们了解用户环境并优化产品。数据**每个版本收集一次** — 当你升级 Potato 后，会重新收集以便我们了解版本分布。
...
交互式运行 `potato init` 时，会询问你是否同意。使用 `--defaults` 模式则自动同意。提示每个版本仅出现一次，且不影响 Potato 的任何功能。

---

## 许可证

Potato 采用 [Apache License 2.0](LICENSE) 开源协议。

---

## 贡献者

感谢所有为 Potato 做出贡献的朋友们：

<a href="https://github.com/fly-go-run/Potato/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=fly-go-run/Potato" alt="贡献者" />
</a>
