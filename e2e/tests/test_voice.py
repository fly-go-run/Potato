# -*- coding: utf-8 -*-
"""
Potato Voice Transcription module P0 end-to-end tests
    ...
    def test_voice_config_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            page.wait_for_selector(
                '.potato-radio-group, .potato-radio-wrapper, '
                '.potato-switch, '
                'h1:has-text("Voice"), .potato-page-header:has-text("Voice")',
                timeout=15000,
            )
        except Exception:
            logger.warning(
        ...
        page_title = page.locator('h1:has-text("Voice"), .potato-page-header:has-text("Voice")').first
        ...
        radio_group = page.locator('.potato-radio-group, .potato-radio-wrapper').first
        voice_switch = page.locator('.potato-switch').first
        ...
        all_controls = page.locator('.potato-radio-wrapper, .potato-switch, .potato-select, input').all()
        ...
        form_fields = page.locator('.potato-form-item, .potato-radio-wrapper, input, .potato-select, textarea').all()
        ...
        if has_radio:
            radio_items = page.locator('.potato-radio-wrapper').all()
        ...
        save_btn = page.locator('button:has-text("保存"), button:has-text("Save"), button.potato-btn-primary').first
...
class TestVoiceToggle:
    ...
    def test_voice_service_toggle(self, page: Page, request: pytest.FixtureRequest):
        ...
        voice_toggle = page.locator('.potato-switch').first
        ...
        all_controls = page.locator(
            '.potato-radio-group, .potato-radio-wrapper, .potato-switch, '
            '.potato-select, .ant-select, input, select, textarea, '
            '[class*="card"], .potato-card'
        ).all()
        ...
        if has_radio:
            ...
            checked_radio = page.locator('.potato-radio-wrapper-checked, .potato-radio-wrapper.potato-radio-wrapper-checked').first
            initial_text = ""
            ...
            new_checked = page.locator('.potato-radio-wrapper-checked, .potato-radio-wrapper.potato-radio-wrapper-checked').first
            ...
            voice_select = page.locator('.potato-select, .ant-select').first
            if voice_select.count() > 0 and voice_select.is_visible():
                voice_select.click()
                page.wait_for_timeout(1000)
                # Check dropdown options
                dropdown_items = page.locator('.potato-select-item, .ant-select-item, [class*=select-item]').all()
...
class TestVoiceServiceConfig:
    ...
    def test_twilio_config_form(self, page: Page, request: pytest.FixtureRequest):
        ...
        twilio_section = page.locator('[class*=twilio], .potato-card:has-text("Twilio"), .potato-collapse:has-text("Twilio")').first
        page_content = page.locator('body').inner_text()
        ...
        all_inputs = page.locator('input[type="text"], input[type="password"], .potato-input input, input').all()
        ...
        if len(visible_inputs) > 0:
            ...
            selects = page.locator('.potato-select, .ant-select').all()
        ...
        save_btn = page.locator('button:has-text("Save"), button:has-text("保存"), button.potato-btn-primary').first
        if save_btn.count() > 0 and save_btn.is_visible(timeout=3000):
            assert save_btn.is_enabled(), "Save button should be enabled"
            logger.info("Save button exists and is enabled")
        else:
        ...
        webhook_url = page.locator('[class*=webhook], .potato-paragraph:has-text("/voice/")').or_(page.get_by_text("Webhook", exact=False)).first
...
class TestVoiceModeSwitch:
    ...
    def test_voice_mode_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        mode_select = page.locator(
            '.potato-select, .ant-select, '
            '.potato-radio-group, .potato-segmented'
        ).first
        if mode_select.count() > 0:
            logger.info("Audio mode selector exists")
            if mode_select.locator('.potato-select-selector').count() > 0:
                mode_select.click()
                page.wait_for_timeout(500)
        ...
        switches = page.locator('.potato-switch').all()
        # Voice page should have at least some config controls
        all_controls = page.locator('.potato-switch, .potato-select, .ant-select, input, .potato-radio-group').all()
