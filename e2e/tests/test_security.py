# -*- coding: utf-8 -*-
"""
Potato Security module P0-level end-to-end test cases.
...
class TestSecurityToolGuardAndTabSwitch:
    ...
    def test_tool_guard_toggle_and_tab_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        if not active_panel.is_visible():
            ...
            active_panel = page.locator('.potato-tabs-tabpane-active').first
        ...
        tool_guard_switch = active_panel.locator('button.potato-switch[role="switch"]').first
        ...
        protected_tools_select = active_panel.locator('.potato-select').first
        ...
        file_guard_switch = file_guard_panel.locator('button.potato-switch[role="switch"]').first
...
class TestSecurityFileGuardPathAndToolSelect:
    ...
    def test_file_guard_path_add_and_tool_select(self, page: Page, request: pytest.FixtureRequest):
        ...
        add_button = file_guard_panel.locator('button.potato-btn-primary').first
        ...
        protected_tools_select = tool_guard_panel.locator('.potato-select').first
        expect(protected_tools_select).to_be_visible(timeout=5000)
        # Click the selector inside the Select to trigger the dropdown
        select_selector = protected_tools_select.locator('.potato-select-selector').first
        ...
        dropdown = page.locator('.potato-select-dropdown:visible').first
        if dropdown.count() > 0 and dropdown.is_visible():
            options = dropdown.locator('.potato-select-item').all()
...
class TestSecurityConfigSaveAndPersist:
    ...
    def test_security_config_save_and_persist(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_switch = tool_guard_panel.locator('button.potato-switch[role="switch"]').first
        expect(tool_guard_switch).to_be_visible(timeout=5000)
        ...
        try:
            ...
            save_btn = page.locator('button.potato-btn-primary:has-text("保存"), button:has-text("保 存")').first
            if not save_btn.is_visible():
                save_btn = page.locator('div[class*="footer"] button.potato-btn-primary').first
            ...
            success_msg = page.locator('.potato-message-success, .potato-message-notice-content:has-text("保存")').first
            ...
            tool_guard_tab_refreshed = page.locator('[data-node-key="toolGuard"] .potato-tabs-tab-btn').first
            expect(tool_guard_tab_refreshed).to_be_visible(timeout=5000)
            tool_guard_tab_refreshed.click()
            page.wait_for_timeout(1500)

            tool_guard_panel_refreshed = page.locator('.potato-tabs-tabpane-active').first
            expect(tool_guard_panel_refreshed).to_be_visible(timeout=5000)

            tool_guard_switch_refreshed = tool_guard_panel_refreshed.locator('button.potato-switch[role="switch"]').first
            ...
            try:
                tool_guard_switch_refreshed = page.locator('.potato-tabs-tabpane-active').first.locator('button.potato-switch[role="switch"]').first
                if tool_guard_switch_refreshed.is_visible():
                    ...
                    if current_state != initial_checked:
                        ...
                        # Save again
                        save_btn_refreshed = page.locator('button.potato-btn-primary:has-text("保存"), button:has-text("保 存")').first
                        if not save_btn_refreshed.is_visible():
                            save_btn_refreshed = page.locator('div[class*="footer"] button.potato-btn-primary').first
        ...
        skill_scanner_tab = page.locator('[data-node-key="skillScanner"] .potato-tabs-tab-btn').first
        ...
        if skill_scanner_tab.is_visible():
            ...
            skill_scanner_panel = page.locator('.potato-tabs-tabpane-active').first
...
class TestSecurityRuleCrud:
    ...
    def test_security_rule_crud(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            modal = page.locator('.potato-modal').first
            ...
            confirm_btn = modal.locator('button.potato-btn-primary').first
            ...
            severity_tag = rule_row.locator('.potato-tag:has-text("HIGH")').first
            if severity_tag.count() == 0:
                severity_tag = rule_row.locator('.potato-tag').first
            ...
            enable_switch = rule_row.locator('button.potato-switch[role="switch"]').last
            ...
            edit_modal = page.locator('.potato-modal').first
            ...
            if rule_id:
                try:
                    ...
                    if cleanup_rule_row.count() > 0:
                        ...
                        delete_btn.click()
                        page.wait_for_timeout(1500)
                        confirm_delete_btn = page.locator('.potato-modal-confirm button.potato-btn-primary, .potato-modal button:has-text("确认"), .potato-modal button:has-text("Delete")').first
...
class TestSkillScannerModeSwitch:
    ...
    def test_skill_scanner_mode_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        mode_select = skill_scanner_panel.locator('.potato-select').first
        ...
        current_mode_selector = mode_select.locator('.potato-select-selector').first
        ...
        try:
            ...
            block_option = page.locator('.potato-select-item:has-text("block"), .potato-select-item:has-text("Block")').first
            ...
            warn_option = page.locator('.potato-select-item:has-text("warn"), .potato-select-item:has-text("Warn")').first
            ...
            off_option = page.locator('.potato-select-item:has-text("off"), .potato-select-item:has-text("Off")').first
            ...
            timeout_input = skill_scanner_panel.locator('input[type="number"], .potato-input-number input').first
            ...
            scan_alerts_tab = skill_scanner_panel.locator('[data-node-key="scanAlerts"] .potato-tabs-tab-btn, .potato-tabs-tab-btn:has-text("扫描警报"), .potato-tabs-tab-btn:has-text("Scan Alerts")').first
            ...
            whitelist_tab = skill_scanner_panel.locator('[data-node-key="whitelist"] .potato-tabs-tab-btn, .potato-tabs-tab-btn:has-text("白名单"), .potato-tabs-tab-btn:has-text("Whitelist")').first
            ...
            if original_mode_text and current_mode_selector:
                try:
                    current_mode_selector.click()
                    page.wait_for_timeout(1000)
                    restore_option = page.locator(f'.potato-select-item:has-text("{original_mode_text}")').first
...
class TestDeniedToolsConfig:
    ...
    def test_denied_tools_config(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_tab = page.locator(
            '[data-node-key="toolGuard"] .potato-tabs-tab-btn, '
            '.potato-tabs-tab-btn:has-text("Tool Guard"), '
            '.potato-tabs-tab-btn:has-text("工具防护")'
        ).first
        ...
        active_panel = page.locator('.potato-tabs-tabpane-active').first
        guard_switch = active_panel.locator('.potato-switch').first
        ...
        denied_tools_select = page.locator(
            '#denied_tools, '
            '.potato-select:near(:text("Denied"), 200), '
            '.potato-select:near(:text("拒绝"), 200)'
        ).first
        ...
        if denied_tools_select.count() > 0:
            ...
            options = page.locator('.potato-select-item-option').all()
            if len(options) > 0:
                ...
                # Verify selection
                selected_tags = page.locator('.potato-select-selection-item').all()
            ...
            form_items = active_panel.locator('.potato-form-item').all()
...
class TestRulePreview:
    ...
    def test_rule_preview(self, page: Page, request: pytest.FixtureRequest):
        ...
        active_panel = page.locator('.potato-tabs-tabpane-active').first
        rule_table = active_panel.locator('table, .potato-table').first
        ...
        if rule_table.count() > 0:
            ...
            # Count rule rows
            rule_rows = active_panel.locator('table tbody tr, .potato-table-row').all()
            ...
            if len(preview_btns) > 0:
                ...
                preview_modal = page.locator('.potato-modal').first
                if preview_modal.count() > 0:
                    ...
                    # Close the modal
                    close_btn = preview_modal.locator('.potato-modal-close, button:has-text("Close"), button:has-text("关闭"), button:has-text("OK")').first
...
class TestSecurityBatchRuleToggle:
    ...
    def test_security_batch_rule_toggle(self, page: Page, request: pytest.FixtureRequest):
        ...
        log_test_step("Find the rules table")
        active_panel = page.locator('.potato-tabs-tabpane-active').first
        rule_switches = active_panel.locator('.potato-switch').all()
