# -*- coding: utf-8 -*-
"""
Potato Environments module P0 end-to-end test cases.
...
CHECKBOX_SELECTOR = '.potato-checkbox-input'
COUNT_SELECTOR = 'span[class*="toolbarCount"]'
SAVE_BTN_SELECTOR = 'button.potato-btn-primary:has-text("保存"), button:has-text("保 存"), button:has-text("Save")'
...
def click_save_button(page: Page):
    ...
    if not save_btn.is_visible(timeout=3000):
        ...
        fallback_selectors = [
            'div[class*="toolbar"] button.potato-btn-primary',
            'button.potato-btn-primary:visible',
            'button:has-text("保存")',
...
class TestEnvironmentListDisplay:
    ...
    def test_environment_list_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        if row_count > 0:
            ...
            empty_css = page.locator('.potato-empty, [class*=empty]').first
...
class TestAddEnvironment:
    ...
    def test_add_environment_key_required(self, page: Page, request: pytest.FixtureRequest):
        ...
        error_css = page.locator(
            '.potato-form-item-validate-error, .potato-message-error, '
            '.potato-form-item-explain-error, .potato-message-notice-content'
        ).first
    ...
    def test_env_var_save_and_persist(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            success_msg = page.locator(
                ...
                '.potato-message-notice-content:has-text("保存")'
            ).first
...
class TestEnvVarKeyValidation:
    ...
    def test_env_var_key_format_validation(self, page: Page, request: pytest.FixtureRequest):
        ...
        error_selector = '.potato-form-item-explain-error, [class*="error"]:has-text("格式")'
    ...
    def test_env_key_duplicate_detection(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            if len(key_inputs) == 0:
                ...
                row_inputs = page.locator('tr:last-child input, .potato-form-item input').all()
            ...
            error_indicators = page.locator(
                '.potato-form-item-has-error, '
                '[style*="border-color: red"], '
                '[style*="color: red"], '
                '.potato-form-item-explain-error, '
