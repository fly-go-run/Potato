...
class ChannelsPage(BasePage):
    ...
    CHANNEL_DRAWER = '.potato-drawer:visible, .ant-drawer:visible'
    DRAWER_TITLE = '.potato-drawer-title, .ant-drawer-title'
    DRAWER_CLOSE_BTN = '.potato-drawer-close, .ant-drawer-close'

    # Form fields
    FORM_ITEM = '.ant-form-item, .potato-form-item'
    FORM_LABEL = '.ant-form-item-label, .potato-form-item-label'
    FORM_INPUT = 'input.ant-input, input.potato-input'
    FORM_SWITCH = '.ant-switch, .potato-switch'
    FORM_SELECT = '.ant-select-selector, .potato-select-selector'
    FORM_SUBMIT_BTN = '.potato-drawer button:has-text("保 存"), .potato-drawer button:has-text("保存"), .potato-drawer button:has-text("Save"), .ant-drawer button:has-text("Save")'
    FORM_CANCEL_BTN = '.potato-drawer button:has-text("取 消"), .potato-drawer button:has-text("取消"), .potato-drawer button:has-text("Cancel"), .ant-drawer button:has-text("Cancel")'

    # Channel-specific field selectors (composed dynamically per channel type)
    BOT_PREFIX_INPUT = '.potato-drawer input[placeholder*="@bot"], .potato-drawer input[placeholder*="bot prefix" i], input[placeholder*="Bot Prefix" i], input[placeholder*="机器人前缀" i]'
    ...
    # src/potato/app/channels/registry.py::_BUILTIN_SPECS).
    #
    ...
    def wait_for_drawer_open(self, timeout: Optional[int] = None) -> bool:
        ...
        try:
            self.page.locator('.potato-drawer, .ant-drawer').first.wait_for(state="visible", timeout=timeout)
            return True
        except Exception:
            return False
    ...
    def toggle_enable(self, enable: bool = True) -> "ChannelsPage":
        ...
        drawer = self.page.locator('.potato-drawer, .ant-drawer')
        switch = drawer.locator('.potato-switch, .ant-switch').first
    ...
    def has_form_validation_errors(self) -> bool:
        ...
        errors = self.page.locator(
            '.potato-form-item-explain-error, .ant-form-item-explain-error'
        )
        count = errors.count()
        if count > 0:
