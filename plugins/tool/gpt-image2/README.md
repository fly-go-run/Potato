...
A Potato tool plugin that enables image generation and editing using OpenAI's GPT Image 2 model.
...
```bash
potato plugin install /path/to/gpt-image2
```

Or from ZIP:

```bash
potato plugin install gpt-image2-tool.zip
```

## Configuration

1. Start Potato application
...
- Ensure the plugin is installed: `potato plugin list`
- Check Potato logs: `~/.potato/logs/potato.log`
- Restart Potato after installation
...
- Check file permissions in `~/.potato/plugins/`
...
For issues and feature requests, please use the Potato issue tracker.
