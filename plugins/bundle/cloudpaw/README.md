<p align="center">
  <img src="https://raw.githubusercontent.com/fly-go-run/Potato/main/plugins/bundle/cloudpaw/docs/cloudpaw.png" alt="CloudPaw" width="360" />
</p>

<p align="center">
  <strong>Cloud Capability Enhancement Plugin for Potato</strong>
</p>
...
CloudPaw is a cloud capability enhancement plugin for Potato, combining **Potato + Aliyun CLI** with deep **IaC** integration. It's not just a chatbot — it's an intelligent assistant with a cloud-native execution engine.
...
| Item | Requirement |
|------|-------------|
| **Potato version** | **≥ v1.1.7** |
| **Python** | 3.10 ~ 3.13 |
| **Alibaba Cloud account** | Access Key required for cloud operations |

> For Potato installation, see [Potato Quickstart](https://qwenpaw.agentscope.io/docs/quickstart). If your Potato version is below v1.1.7, upgrade first: `pip install --upgrade qwenpaw>=1.1.7`.

### 1. Install CloudPaw Plugin

**Via Console (recommended):**

1. Launch Potato (`potato app`), open http://127.0.0.1:8088/
2. Click "Plugin Manager" in the left sidebar (under Settings), then click "Install Plugin"
3. Install by dragging the `cloudpaw/` folder into the install dialog, or selecting a ZIP file (CloudPaw is pre-bundled with Potato v1.1.7+ at `plugins/bundle/cloudpaw/`)
4. Wait for installation to complete

**Via CLI:**

```bash
potato plugin install /path/to/cloudpaw
```
...
Configure an LLM provider and API Key in console Settings → Models. See [Potato Models docs](https://qwenpaw.agentscope.io/docs/models).
...
CloudPaw relies on [iac-code](https://github.com/aliyun/iac-code) (≥ 0.1.2) for IaC template generation. **No manual model configuration is needed** — CloudPaw automatically syncs Potato's active model to iac-code.

When the CloudPaw plugin starts, it writes `llm_source: qwenpaw` to `~/.iac-code/settings.yml`. This tells iac-code to read model configuration (provider, API key, model name, etc.) directly from Potato's active model. As long as you have configured a working model in Potato (step ①), iac-code will use the same model automatically — no extra setup required.

**Manual override:** If you need iac-code to use a different model from Potato, set the `IAC_CODE_PROVIDER` environment variable (via Potato's Environment Variables page or system env). When this variable is present, CloudPaw skips automatic injection and iac-code uses your manual configuration. For details, see the [iac-code LLM configuration docs](https://aliyun.github.io/iac-code/docs/configuration/llm-providers).
...
CloudPaw integrates via the Potato native plugin system.
...
- **PRD Management Frontend Enhancement**: Custom frontend rendering for Potato Mission Mode's PRD management (`manage_prd` tool)
- **Multi-Agent Collaboration**: Orchestrate multiple agents for complex deployment tasks via Potato Mission Mode
...
CloudPaw implements multi-agent collaboration via Potato's **Mission Mode**. Users interact with the master agent, which automatically breaks down requirements into a PRD (Product Requirements Document) and delegates tasks to specialized sub-agents by story priority.
