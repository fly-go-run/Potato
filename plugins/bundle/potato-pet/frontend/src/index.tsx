// React and antd are injected by the Potato console host at runtime;
...
//
// ``potato-host.d.ts`` declares the ``window.Potato`` contract so the
 ...
 * blob keyed ``potato-agent-storage`` (sessionStorage takes precedence
 * over localStorage so each tab can pin its own agent). The Potato API
...
function getSelectedAgentId(): string | null {
  try {
    const raw =
      window.sessionStorage?.getItem("potato-agent-storage") ??
      window.localStorage?.getItem("potato-agent-storage");
    if (!raw) return null;
...
function PetThumb({ folder }: { folder: string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const url = getApiUrl(
          `/potato-pet/pets/${encodeURIComponent(folder)}/spritesheet`,
        );
...
function PetControlPage() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      const [petData, st] = await Promise.all([
        apiGet("/potato-pet/pets"),
        apiGet("/potato-pet/status"),
      ]);
  ...
  const startDesktop = async () => {
    ...
    try {
      const r = await apiPost("/potato-pet/desktop/start", {});
      const h = r?.desktop;
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl("/potato-pet/import-pet-upload"), {
        method: "POST",
        headers: authHeaders(),
        body: form,
      });
  ...
  const switchTo = async (row: PetRow) => {
    ...
    try {
      const r = await apiPost("/potato-pet/switch-pet", { pet_id });
      if (r && r.ok === false) {
...
class PotatoPetPlugin {
  readonly id = "potato-pet";

  setup(): void {
    const locale = resolvePetLocale();
    window.Potato.registerRoutes?.(this.id, [
      {
        path: "/plugin/potato-pet/pets",
