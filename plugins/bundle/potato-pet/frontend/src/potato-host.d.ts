// Ambient declarations for the Potato console host API.
//
// The Potato console injects a shared `window.Potato` object at
...
declare global {
  interface PotatoHost {
    ...
    /** Current bearer token for Potato API calls (may be empty). */
    getApiToken: () => string;
  }

  interface PotatoRoute {
    path: string;
    component: unknown;
    label?: string;
    icon?: string;
  ...
  interface PotatoGlobal {
    host: PotatoHost;
    registerRoutes?: (pluginId: string, routes: PotatoRoute[]) => void;
  }
