# Potato Pet (plugin + desktop runtime)

Full-stack Potato plugin: backend hooks, console UI sidebar, **and** the
desktop pet runtime itself (`potato_pet_desktop/`). Installing this
plugin gives you everything: the Qt floating pet window, the local HTTP
bridge on `127.0.0.1:8765`, and the Potato side hooks that emit
...
├── router.py              # plugin HTTP routes under /api/potato-pet/...
...
└── potato_pet_desktop/   # the Qt + FastAPI desktop runtime (embedded)
    ...
    ├── cli.py             # `python -m potato_pet_desktop` subcommands
...
time, so `potato_pet_desktop` is importable from Potato's Python
process without any `pip install` step. See
[`potato_pet_desktop/README.md`](potato_pet_desktop/README.md) for
...
- **From the Potato console** — open the plugin management page and
  install `potato-pet` like any other Potato plugin. Three input
  shapes are accepted:
...
- **From the shell** — run:
  ...
  ```bash
  potato plugin install ./plugins/potato-pet
  ```

> [!IMPORTANT]
> **Restart Potato after install** — the backend only picks up new
...
Potato's interpreter needs these packages on its `sys.path` (declared
...
If your Potato install does not auto-resolve plugin dependencies,
install them manually into the same environment:

```bash
pip install -r plugins/potato-pet/requirements.txt
```

PySide6-Essentials wheels exist only for **Python 3.10–3.13**. On 3.14
the pet window cannot start; Potato itself will still run, the pet
...
```python
subprocess.Popen(
    [sys.executable, "-m", "potato_pet_desktop.app",
     "--port", "8765"],
...
python -m potato_pet_desktop.app --port 8765 --scale 0.58

# CLI subcommands (daemonized: PID file + log redirect)
python -m potato_pet_desktop start
python -m potato_pet_desktop status
python -m potato_pet_desktop stop
python -m potato_pet_desktop switch --pet-id snowpaw
...
Autostart can be disabled with `POTATO_PET_AUTOSTART=0`.
...
`~/.potato/plugins/potato-pet/dist/index.js` and refresh the console.
...
host at runtime (`window.Potato.host.React`, `host.antd`,
`host.getApiUrl`, `host.getApiToken`). The shape of this contract is
declared in `frontend/src/potato-host.d.ts`. If the console host
...
The UI adds a sidebar page **Pet** (`/plugin/potato-pet/pets`): lists
pets under `<WORKING_DIR>/pets`, **Start desktop pet** (calls `POST
/api/potato-pet/desktop/start`), **Switch** (hot-switch via `POST
...
`POST /api/potato-pet/import-pet-upload`, then validated and copied
...
## Potato plugin HTTP routes

```text
GET  /api/potato-pet/status
GET  /api/potato-pet/pets
GET  /api/potato-pet/pets/{folder}/spritesheet
POST /api/potato-pet/desktop/start
POST /api/potato-pet/switch-pet
POST /api/potato-pet/import-pet          # JSON body — server-side path
POST /api/potato-pet/import-pet-upload   # multipart/form-data — browser
POST /api/potato-pet/emit-test
```
...
`X-Potato-Pet-Token: <runtime/update-token>` by default — the bundled
Potato plugin reads the token file automatically; standalone clients
must do the same. Set `POTATO_PET_REQUIRE_TOKEN=0` to disable the check
...
Row layout (driven by `potato_pet_desktop/sprites.py`):
...
(`potato_pet_desktop/assets/default-pet/snowpaw/`).
...
installed and cached under `~/.potato-pet/cache/snowpaw-spritesheet.webp`
for subsequent runs. Override the source with `POTATO_PET_SNOWPAW_URL`
...
`potato_pet_desktop/assets/default-pet/snowpaw/` and the bundled copy
...
- `register_startup_hook` — patches `AgentRunner.query_handler` and the
  ...
  `potato.startup`. Patch failures (e.g. an upstream rename of
  `AgentRunner` / `ApprovalService`) are reported via
  `logger.exception` so a broken plugin install does not stay silently
  dead.
- `register_shutdown_hook` — emits `potato.shutdown`, terminates the
  pet desktop process that this Potato process has adopted (either
  autostarted by the plugin or already healthy on startup /
  `desktop/start`; controlled by `POTATO_PET_STOP_ON_SHUTDOWN`), and
  restores the patched class methods. So when the user exits Potato
  the floating pet exits with it, including the case where the pet was
  a leftover from a previous Potato run.
- `register_http_router` — mounts `router.py` under `/potato-pet`.

## Environment variables

| Var | Purpose | Default |
| --- | --- | --- |
| `POTATO_PET_DESKTOP_URL` | Full base URL for the plugin → desktop HTTP bridge. If set, port auto-fallback is **disabled** (URL and listener must agree). | unset ⇒ derive from host + port below |
| `POTATO_PET_DESKTOP_HOST` | Bind address when spawning (`--host`) | `127.0.0.1` |
| `POTATO_PET_DESKTOP_PORT` | Preferred port when spawning; if busy, the plugin scans upward (unless `DESKTOP_URL` is set or `STRICT` is on) | `8765` |
| `POTATO_PET_DESKTOP_STRICT_PORT` | `1` ⇒ never auto-pick another port (fail with EADDRINUSE if taken) | `0` |
| `POTATO_PET_DESKTOP_SCALE` | Spawn-time scale (e.g. `0.58`) | unset |
| `POTATO_PET_DESKTOP_PET_DIR` | Spawn-time pet folder override | unset |
| `POTATO_PET_TOKEN_PATH` | Path to the local update token | `~/.potato-pet/runtime/update-token` |
| `POTATO_PET_REQUIRE_TOKEN` | `0` ⇒ desktop *skips* the token check on mutating endpoints (anything else, including unset, enforces it) | `1` |
| `POTATO_PET_AUTOSTART` | `0` ⇒ plugin will not spawn the desktop | `1` |
| `POTATO_PET_STOP_ON_SHUTDOWN` | `0` ⇒ leave the pet desktop running after Potato exits. Default: terminate any pet desktop Potato has adopted (either by autostarting it or by seeing it healthy at startup / explicit `desktop/start`). | `1` |
| `POTATO_PET_HOME` | Runtime dir (PID file, log, **cache**, token) | `~/.potato-pet/` |
| `POTATO_PET_SNOWPAW_URL` | CDN URL for snowpaw's `spritesheet.webp` (downloaded once on first install) | Alicdn-hosted default |
| `POTATO_WORKING_DIR` (legacy: `QWENPAW_WORKING_DIR` / `COPAW_WORKING_DIR`) | Where `pets/` lives | reuses an existing `~/.qwenpaw` or `~/.copaw`, otherwise `~/.potato` |

### Pet desktop log / common errors

`~/.potato-pet/runtime/pet-desktop.log` captures stderr from the spawned
desktop process.

- **`ModuleNotFoundError: No module named 'PySide6'`** — install Qt into the
  **same** Python as Potato:
...
- **`[Errno 48] address already in use` (uvicorn)** — something else is bound
  ...
  that process, set `POTATO_PET_DESKTOP_PORT` to a free port, or unset
  `POTATO_PET_DESKTOP_URL` and let the plugin auto-pick the next free port
  after the preferred one.

The effective listen URL is mirrored under
`~/.potato-pet/runtime/desktop-bridge.json` (`url` field) so the plugin can
