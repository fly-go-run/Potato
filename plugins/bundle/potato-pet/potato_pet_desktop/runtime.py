...
def home_dir() -> Path:
    explicit = os.environ.get("POTATO_PET_HOME") or os.environ.get(
        "QWENPAW_PET_HOME",
    )
    if explicit:
        return Path(explicit)
    current = Path.home() / ".potato-pet"
    legacy = Path.home() / ".qwenpaw-pet"
    if not current.exists() and legacy.exists():
        return legacy
    return current


def potato_working_dir() -> Path:
    """Return Potato's working directory using Potato's precedence."""
    explicit = (
        os.environ.get("POTATO_WORKING_DIR")
    ...
    try:
        from potato.constant import WORKING_DIR  # type: ignore

        return Path(WORKING_DIR).expanduser().resolve()
    except Exception:
        for legacy_name in (".qwenpaw", ".copaw"):
            legacy = Path.home() / legacy_name
            if legacy.exists():
                return legacy.resolve()
        return Path("~/.potato").expanduser().resolve()
...
def pets_dir() -> Path:
    return potato_working_dir() / "pets"
...
def current_process_status() -> dict[str, Any]:
    ...
    return {
        ...
        "home": str(home_dir()),
        "potatoWorkingDir": str(potato_working_dir()),
