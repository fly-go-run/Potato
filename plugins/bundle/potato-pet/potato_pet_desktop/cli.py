# -*- coding: utf-8 -*-
"""Command-line management for Potato Pet Desktop."""
...
def command_init(_args: argparse.Namespace) -> int:
    ...
    print(f"Potato working dir: {runtime.potato_working_dir()}")
    ...
    print("")
    print("Install the Potato backend plugin with:")
    print("  potato plugin install ./plugins/potato-pet")
    return 0
...
def command_start(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if health.get("ok"):
            print(f"Potato Pet already running on port {args.port}")
            return 0
    except Exception:
        pass
    ...
    print(f"Started Potato Pet Desktop (pid {process.pid})")
...
def command_switch(args: argparse.Namespace) -> int:
    ...
    if token:
        headers["X-Potato-Pet-Token"] = token
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="potato-pet")
    ...
    g.add_argument(
        "--pet-id",
        help="Folder name under Potato pets/ (e.g. snowpaw)",
    )
    p.add_argument("--port", type=int, default=8765)
