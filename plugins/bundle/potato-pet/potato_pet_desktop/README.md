# `potato_pet_desktop` (package internals)

Importable Python package embedded inside the **Potato Pet plugin**.
It ships the desktop runtime: a transparent always-on-top Qt pet
window + a local FastAPI bridge on `127.0.0.1:8765` that Potato (via
...
the package is importable as `potato_pet_desktop` from Potato's
...
| `__main__.py` | `python -m potato_pet_desktop` → delegates to `cli.main`. |
| `cli.py` | CLI subcommands (`python -m potato_pet_desktop ...`): `init`, `start`, `stop`, `status`, `install-pet`, `install-default-pet`, `token`, `switch`, `send`. |
...
| `server.py` | FastAPI app factory (`build_app`). Defines `/health`, `/event`, `/state`, `/bubble`, `/pet`. Optional bearer-style token guard via `POTATO_PET_REQUIRE_TOKEN=1` + `X-Potato-Pet-Token`. |
...
| `assets/default-pet/snowpaw/` | The bundled default pet (`pet.json` + `spritesheet.webp`). Loaded via `importlib.resources.files("potato_pet_desktop")`, which resolves relative to this folder on disk (no setuptools install required). |

## Runtime layout

`runtime.home_dir()` defaults to `~/.potato-pet/` (override with
`POTATO_PET_HOME`) and contains:
...
~/.potato-pet/runtime/
  ...
  pet-desktop.log    # stdout/stderr of detached `potato-pet start`
```

Pet **packages** live in a separate location resolved by
`runtime.potato_working_dir()`, with this precedence:

1. `POTATO_WORKING_DIR`
2. `QWENPAW_WORKING_DIR`, then `COPAW_WORKING_DIR`
3. `from potato.constant import WORKING_DIR` (if Potato is installed)
4. `~/.qwenpaw`, then `~/.copaw`, if either already exists
5. `~/.potato`
...
send `X-Potato-Pet-Token: <token from runtime/update-token>` on
mutating endpoints (the bundled Potato plugin reads the token file
automatically). To disable the check (e.g. for local development), set
`POTATO_PET_REQUIRE_TOKEN=0`.
...
```text
potato.startup   -> waving
potato.shutdown  -> idle
...
These two are equivalent and bypass the `potato-pet start` daemon
wrapper (no PID file, no log redirection — useful while iterating):

```bash
python -m potato_pet_desktop.app --port 8765 --scale 0.58
python -m potato_pet_desktop                 # → cli.main, takes subcommands
```
