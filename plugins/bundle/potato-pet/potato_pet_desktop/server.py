# -*- coding: utf-8 -*-
"""Local HTTP bridge for Potato lifecycle events."""
...
def _token_required() -> bool:
    """Whether the local bridge enforces ``X-Potato-Pet-Token``.
    ...
    setting ``POTATO_PET_REQUIRE_TOKEN=0``.
    """
    return os.environ.get("POTATO_PET_REQUIRE_TOKEN", "1") != "0"
...
def build_app(
    ...
    app = FastAPI(title="Potato Pet Desktop")
    ...
    async def event(
        payload: PetEvent,
        request: Request,
        x_potato_pet_token: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _check_token(x_potato_pet_token)
