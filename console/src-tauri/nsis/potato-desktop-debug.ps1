...
$desktopExe = Join-Path $PSScriptRoot "potato-desktop.exe"
$stdoutLog = Join-Path $env:TEMP "potato-desktop-debug.stdout.log"
$stderrLog = Join-Path $env:TEMP "potato-desktop-debug.stderr.log"
...
$desktop = Get-Process -Name "potato-desktop" -ErrorAction SilentlyContinue |
  Where-Object { $_.Path -eq $desktopExe } |
  Select-Object -First 1
if ($desktop) {
  Write-Host "Existing Potato Desktop process detected; quit it from the tray before reproducing startup issues."
  $desktop = $null
} else {
...
$specs = @(
  [pscustomobject]@{ Label = "backend"; Paths = $env:POTATO_BACKEND_LOGS -split ";" },
  [pscustomobject]@{ Label = "shell"; Paths = $env:POTATO_SHELL_LOGS -split ";" }
)
$states = @()
  ...
  while ($true) {
    ...
    if ($desktop) {
      ...
      if ($desktop.HasExited) {
        Write-Host "Potato Desktop exited with code $($desktop.ExitCode)."
        Show-DesktopOutput
