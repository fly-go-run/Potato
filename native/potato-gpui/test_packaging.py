...
class Registry:
    ...
    def OpenKey(self, _hive, name, _reserved, access):
...
class PackagingTests(unittest.TestCase):
    # unittest owns the context across setUp, the test and tearDown.
    # pylint: disable=consider-using-with
    def setUp(self):
        self.work = Path(
            self.enterContext(
                tempfile.TemporaryDirectory(prefix="potato packaging test "),
            ),
        )
    ...
    def test_complete_assets_and_crlf_checksums(self):
        self.assertEqual(
            len(release.verify_assets(self.packages, self.version)),
            12,
        )
    ...
    def test_corruption_and_checksum_target_substitution_are_rejected(self):
        ...
        checksum.write_text(
            checksum.read_text().replace(archive.name, "other.zip"),
        )
        with self.assertRaises(ValueError):
            release.verify_assets(self.packages, self.version)
        checksum.write_text(
    ...
    def test_existing_custom_or_legacy_installation_is_never_touched(self):
        ...
        for key in (INSTALL_KEY, UNINSTALL_KEY):
            for view in (Registry.KEY_WOW64_64KEY, Registry.KEY_WOW64_32KEY):
                with self.subTest(key=key, view=view), self.assertRaises(
                    RuntimeError,
                ):
                    assert_no_windows_installation(
                        Registry({(key, view)}),
    ...
    def test_public_release_or_api_failure_cannot_be_overwritten(self):
        existing = [
            [{"tag_name": "native-v1.2.3-rc.1", "draft": False, "assets": []}],
        ]
        with patch.object(
            release,
            "gh",
    ...
    def simulate_publish(self, *, corrupt=False, existing=False):
        ...
        def gh(*args):
                ...
                return json.dumps(
                    {
                        ...
                        "assets": [
                            {"name": n}
                            for n in release.asset_names(self.version)
                        ],
                    },
                )
            if args[:2] == ("release", "download"):
                ...
                if corrupt:
                    next(destination.glob("*.zip")).write_bytes(
                        b"corrupt remote download",
                    )
            return ""
    ...
    def test_publish_happens_only_after_readback(self):
        ...
        self.assertLess(
            calls.index(("release", "download")),
            calls.index(("release", "edit")),
        )

    def test_corrupt_upload_leaves_unpublished_draft(self):
        self.assertNotIn(
            ("release", "edit"),
            self.simulate_publish(corrupt=True),
        )
