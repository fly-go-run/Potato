            ...
            .when(!options.todo && checked.is_none(), |this| {
                this.child(
                    div()
                        ...
                        .pr(rems(0.75))
                        .text_right()
                        .child(list_item_prefix(ix, options.ordered, options.depth).trim_end().to_owned()),
                )
            })
                ...
                .children({
                    ...
                    for (child_ix, child) in children.iter().enumerate() {
                        match child {
                            ...
                            BlockNode::List { .. } => {
                                items.push(div().ml(rems(2.)).child(child.render_block(
                                    NodeRenderOptions {
                                        depth: options.depth + 1,
                                        todo: checked.is_some(),
                                        is_last: true,
                            ...
                            | BlockNode::HorizontalRule { .. } => {
                                ...
                                if child_ix == 0 {
                                    ...
                                    // nested sub-list (`ml(rems(2.))`) and with
    ...
    pub(crate) fn render_block(
        ...
        match self {
            ...
            BlockNode::HorizontalRule { .. } => div()
                // The preceding paragraph/list already contributes one rem.
                .pt(rems(1.))
                .pb(rems(2.))
                .child(
                    div()
