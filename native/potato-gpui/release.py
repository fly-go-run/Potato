...
def asset_names(release_version):
    ...
    packages = [
        f"{prefix}-Windows-x86_64-{suffix}"
...
def verify_assets(directory, release_version):
    ...
    if actual != expected:
        raise ValueError(
            f"Missing release assets: {sorted(set(expected) - set(actual))}, "
    ...
    for name in expected:
        ...
        if not name.endswith(".sha256"):
            ...
            if (
                ...
                raise ValueError(
                    f"Invalid checksum or filename: {name}",
                )
    return digests
...
- macOS 15+: choose arm64 (Apple Silicon) or x86_64 (Intel), open the DMG and
  drag Potato GPUI to Applications.
- Python and WebView are not required to run the application. The native
  computer driver is bundled.
- Packages are not developer-signed/notarized; OS trust prompts may appear.
- CI checks packaged core startup and Windows install, upgrade, in-use
  rejection and uninstall. GPU rendering, IME and microphone interaction
  still need real-device testing.
- User data is retained on uninstall. This preview does not use the Tauri
  updater feed.
"""
...
def gh(*args):
    ...
    if result.returncode:
        raise RuntimeError(
            f"gh failed ({result.returncode}): {result.stderr.strip()}",
        )
...
def publish(directory, tag, repository, release_version):
    ...
    # `gh release view` returns a generic nonzero status for all four.
    pages = json.loads(
        gh(
            "api",
            "--paginate",
    ...
    existing = next(
        (r for page in pages for r in page if r["tag_name"] == tag),
        None,
    )
    ...
    if existing is not None:
        ...
        if unexpected:
            raise ValueError(
                "Draft contains unexpected assets; review before retrying: "
                f"{sorted(unexpected)}",
            )
    with tempfile.TemporaryDirectory(prefix="potato-release-") as temporary:
        ...
        gh(
            ...
            repository,
            "--clobber",
            *[directory / n for n in sorted(local)],
        )
        remote = json.loads(
            gh(
                "release",
        ...
        if not remote["isDraft"] or sorted(
            a["name"] for a in remote["assets"]
        ) != sorted(local):
            raise ValueError(
                "Remote release changed; leaving it unpublished",
            )
        ...
        if verify_assets(downloaded, release_version) != local:
            raise ValueError(
                "Uploaded assets differ from local packages; "
                "leaving the draft unpublished",
            )
        gh(
            "release",
            "edit",
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing only the 12 release files",
    )
    parser.add_argument(
        "--tag",
        required=True,
        help="native-v<version>; must match Cargo.toml",
    )
    parser.add_argument(
        "--repo",
        help="GitHub owner/repository; required with --publish",
    )
    parser.add_argument(
        "--publish",
        action="store_true",
        help="Upload, read back and publish the GitHub prerelease",
    )
    ...
    if args.publish:
        ...
        print(
            f"Verified {len(asset_names(release_version))} release files "
            f"for {args.tag}",
        )
