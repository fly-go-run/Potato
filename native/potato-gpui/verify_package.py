...
UNINSTALL_KEY = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\PotatoGPUI"


def assert_no_windows_installation(winreg, default_install):
    # Check both registry views, including custom paths and incomplete installs.
    for view in (winreg.KEY_WOW64_64KEY, winreg.KEY_WOW64_32KEY):
        for name in (INSTALL_KEY, UNINSTALL_KEY):
            try:
                with winreg.OpenKey(winreg.HKEY_CURRENT_USER, name, 0, winreg.KEY_READ | view):
                    raise RuntimeError(f"Refusing to touch an existing installation registry key: {name}")
            except FileNotFoundError:
                pass
    if default_install.exists():
        raise RuntimeError(f"Refusing to replace an existing installation: {default_install}")
...
def smoke(binary, data, logs, label, expected):
    ...
    if sys.platform == "win32":
        ...
        for executable in (binary, driver):
            ...
            (logs / f"{label}-{executable.stem}-imports.log").write_text(imports, encoding="utf-8")
            if re.search(r"(?:VCRUNTIME|MSVCP|MSVCR)\d+.*\.dll", imports, re.IGNORECASE):
                raise RuntimeError(f"{executable.name} requires a Visual C++ redistributable")
        if run([str(driver), "--version"]).stdout.strip() != f"cua-driver {DRIVER_VERSION}":
...
def verify_macos(output, work, logs, arch, release_version):
    ...
    expected = {"version": release_version, "os": "macos", "arch": "aarch64" if arch == "arm64" else arch, "computer_driver_available": True, "computer_driver_version": DRIVER_VERSION}
...
def verify_windows(output, work, logs, release_version):
    ...
    assert_no_windows_installation(winreg, Path(os.environ["LOCALAPPDATA"]) / "Programs/Potato GPUI")
    stem = f"Potato-GPUI-{release_version}-Windows-x86_64"
    expected = {"version": release_version, "os": "windows", "arch": "x86_64", "computer_driver_available": True, "computer_driver_version": DRIVER_VERSION}
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, registry, 0, winreg.KEY_READ | winreg.KEY_WOW64_64KEY) as key:
            ...
            for name, expected_command in (
                ...
                ("QuietUninstallString", f'"{installed / "Uninstall.exe"}" /S'),
            ):
                if winreg.QueryValueEx(key, name)[0] != expected_command:
                    raise RuntimeError(f"Invalid registry uninstall command: {name}")
        ...
        for name in ("Potato-GPUI.exe", "computer-driver/cua-driver.exe"):
            with windows_read_lock(installed / name):
                for command in ([str(setup), "/S"],
                                f'"{installed / "Uninstall.exe"}" /S _?={installed}'):
                    result = subprocess.run(command, capture_output=True, timeout=30)
                    if result.returncode == 0:
                        raise RuntimeError(f"Installer/uninstaller accepted an in-use file: {name}")
            after = {p.relative_to(installed): hashlib.sha256(p.read_bytes()).hexdigest()
                     for p in installed.rglob("*") if p.is_file()}
            if before != after:
                raise RuntimeError("Blocked installation/uninstallation changed the payload")
    ...
    (logs / "windows-installation.json").write_text(json.dumps({
        ...
        "custom_unicode_path": True, "in_use_rejected_without_changes": True,
    }), encoding="utf-8")
