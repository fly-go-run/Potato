...
def package_macos(binary, output, arch, release_version, debug):
...
def package_windows(binary, output, release_version, debug):
    if debug:
    ...
    run(makensis, "/V3", f"/DVERSION={release_version}", f"/DNUMERIC_VERSION={numeric_version}",
...
def main():
    ...
    if sys.platform == "darwin":
        artifacts = package_macos(binary, output, arch, release_version, args.debug)
    else:
        artifacts = package_windows(binary, output, release_version, args.debug)
