"""Package GPUI releases. Python 3.11+ and NSIS (Windows) are build tools only."""
...
def package_macos(binary, output, arch, release_version, debug, driver_archive=None):
    ...
    shutil.copyfile(REPO / "console/src-tauri/icons/icon.icns", resources / "icon.icns")
    with (bundle / "Contents/Info.plist").open("wb") as stream:
        plistlib.dump({
            "CFBundleExecutable": "potato-gpui",
            "CFBundleIdentifier": "dev.potato.gpui-review" if debug else "dev.potato.gpui",
            "CFBundleName": name,
    ...
    with tempfile.TemporaryDirectory(prefix="potato-dmg-") as staging:
        ...
        run("hdiutil", "create", "-ov", "-volname", name, "-srcfolder", staged,
            "-format", "UDZO", dmg)
    run("hdiutil", "verify", dmg)
    return [archive, dmg]


def package_windows(binary, output, release_version, debug, driver_archive=None):
    if debug:
    ...
    if not makensis:
        raise RuntimeError("NSIS is required: install NSIS and add makensis.exe to PATH")
    ...
    shutil.copyfile(REPO / "console/src-tauri/icons/icon.ico", payload / "icon.ico")
    shutil.copyfile(REPO / "LICENSE", payload / "LICENSE.txt")
    archive = Path(shutil.make_archive(str(output / f"{stem}-portable"), "zip", output, payload.name))
    installer = output / f"{stem}-setup.exe"
    numeric_version = release_version.split("-")[0] + ".0"
    # /D arguments precede the script; subprocess keeps paths with spaces intact.
    run(makensis, "/V3", "/WX", f"/DVERSION={release_version}", f"/DNUMERIC_VERSION={numeric_version}",
        f"/DPAYLOAD={payload}", f"/DOUTPUT={installer}", ROOT / "installer.nsi")
...
def main():
    ...
    parser.add_argument("--driver-archive", type=Path, help="Local official driver archive; SHA-256 is always verified")
    parser.add_argument("--target", choices=TARGETS, help="Cargo --target triple (omit for host build)")
    ...
    if sys.platform not in ("darwin", "win32") or arch not in ("arm64", "x86_64"):
        parser.error("Supported packages: macOS arm64/x86_64 and Windows x86_64")
    if sys.platform == "win32" and arch != "x86_64":
        parser.error("Windows packaging currently requires x86_64")
    target_dir = Path(os.environ.get("CARGO_TARGET_DIR", ROOT / "target"))
    if args.target:
        target_dir /= args.target
    binary = target_dir / ("debug" if args.debug else "release") / (
    ...
    if sys.platform == "darwin":
        artifacts = package_macos(binary, output, arch, release_version, args.debug, args.driver_archive)
    else:
        artifacts = package_windows(binary, output, release_version, args.debug, args.driver_archive)
