...
// Keep the system instruction and tool description aligned: availability is not a request to run examples.
export const pythonExecutionPolicy = 'Use run_python only when the user explicitly asks to execute or test code, or when fulfilling the requested task requires actual computation, analysis of supplied data/files, or creation of downloadable files/charts. Requests to write, explain, revise, or illustrate code do not by themselves ask for execution. For those requests, provide code without running it; do not run toy inputs merely to demonstrate that an example works. Respect requests not to execute. If execution is needed, call the tool directly; never ask the user to click a code block. When presenting source code and its output together, show the source first and the corresponding output afterward, unless the user asks for another order. Omit output for ordinary examples unless requested or needed to explain the example. Clearly label output that was not executed as expected or illustrative, never as an actual run. Created files are shown to the user as file cards: in the reply, say what they contain, not that they were created, their names, formats, or how to open them.';

export const pythonTool = { type: 'function', function: {
  name: 'run_python',
  description: 'Execute Python in an isolated E2B sandbox. ' + pythonExecutionPolicy + ' Each call rebuilds the sandbox, but files from earlier successful calls in this answer are restored as input files in /home/user/; see available_files in tool results. Send a complete script. No internet, credentials or direct history database access. Use history tools to retrieve permitted past conversations. Save output files in /home/user/output with simple ASCII names. Tool output includes stdout, errors and file names. On a Python error, correct the complete script and retry within the budget.',
  parameters: { type: 'object', properties: { description: { type: 'string', description: 'Short action title in the user language, e.g. Generate the slide deck or Check slide previews. Describe the action, not success; at most 120 characters.', maxLength: 120 }, code: { type: 'string', description: 'Complete Python script, up to 32000 characters.' } }, required: ['code'], additionalProperties: false }
} };
