    ...
    pub(crate) fn outbox_request(&self, body: &Value) -> Result<Value> {
        ...
        if action == "stop" {
            if let Some(expected) = body.get("expected_run_id") {
                let expected = expected.as_str().filter(|id| !id.is_empty())
                    .ok_or_else(|| Error::new(422, "任务编号无效，请刷新"))?;
                if runs.get(session).is_none_or(|run| run.request_id != expected) {
                    return Err(Error::new(412, "原任务已结束或改变，未停止其他任务，请刷新后确认"));
                }
            }
        }
        let db = self.db()?;
        ...
        if q["interrupt"] == true && !matches!(action, "" | "list" | "pause" | "stop") {
        ...
        if action == "stop" { result["stopped"] = json!(runs.contains_key(session)); }
