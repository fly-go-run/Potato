...
2026-09-25 **0.2.2（2026092503）** 已发布至既有 TestFlight「个人内测」组，确认 **Testing**，中文说明读回一致。首次从统一分支 `feat/process-track-codex` 发版（品牌分支已并入，含新图标）。Markdown 表格不再出现空白格（空表头格保留、长单元格换行、窄表格不撑满、`<br>` 与 `\|`），嵌套列表、列表续行、多行引用与标题内联样式；设置页恢复系统控件颜色；远程页状态文案精简并移除旧版草稿恢复。源码 `b3686a88` 加构建号提交，219 项 iOS 单元测试与 Release 签名归档通过。Apple 构建 ID `69863608-508c-419f-aa12-95f5234b3b3c`。说明见 `testflight-notes-2026092503.zh-Hans.txt`。
